--[[
  ...
  Generated: 2025-08-15 16:35:08
  Source: types.xml (this build)
...
-- ============================================================================
--  COMPATIBILITY HELPERS (Lua 3.8 / Lua 4 “pcall returns table” quirk)
--  Injected: 2025-08-15 18:53:37 UTC
-- ============================================================================

--[[
  H55_PCall(func, ...)
  Normalizes pcall across Lua variants used by H5/H55 where pcall may return
  a single table:  result[1] == success boolean, result[2..n] == return values.
  Returns varargs in standard (ok, v1, v2, ...) form for easier usage.
  Example:
    H55_OK, H55_ID = H55_PCall(H55_Resolve, 'HERO', 'TACTICS')
    if H55_OK then ... end
]]
function H55_PCall(func, a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12)
  H55_R1,H55_R2,H55_R3,H55_R4,H55_R5,H55_R6,H55_R7,H55_R8 = H55_PCall(func, a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12)
  if type(H55_R1) == 'table' and H55_R2 == nil then
    H55_T = H55_R1
    return H55_T[1], H55_T[2], H55_T[3], H55_T[4], H55_T[5], H55_T[6], H55_T[7], H55_T[8]
  end
  return H55_R1,H55_R2,H55_R3,H55_R4,H55_R5,H55_R6,H55_R7,H55_R8
end

--[[
  H55_TryResolve(domain, token) -> id or nil
  Safe resolver that never throws; returns nil on failure.
  Usage:
    H55_ID = H55_TryResolve('HERO','TACTICS')
    if H55_ID then GiveHeroSkill('Agrael', H55_ID) end
]]
function H55_TryResolve(domain, token)
  H55_OK, H55_ID = H55_PCall(H55_Resolve, domain, token)
  if H55_OK then return H55_ID end
  return nil
end

--[[
  H55_SafeCall(msg, func, ...)
  Generic safe wrapper: prints a clear error and returns nil on failure.
  msg: string context for dev logs.
  Example:
    H55_SafeCall('GiveHeroSkill', GiveHeroSkill, heroName, skillId)
]]
function H55_SafeCall(msg, func, a1,a2,a3,a4,a5,a6,a7,a8,a9,a10)
  H55_OK,H55_V1,H55_V2,H55_V3 = H55_PCall(func, a1,a2,a3,a4,a5,a6,a7,a8,a9,a10)
  if not H55_OK then
    print('[H55][SAFE]['..tostring(msg)..'] call failed.')
    return nil
  end
  return H55_V1,H55_V2,H55_V3
end
...
function H55_Suggest(domain, tail)
  H55_RES = {}
  H55_MAP = nil
  ...
  while H55_KEY do
    if string.find(H55_KEY, tail, 1, true) then
      H55_RES[H55_Getn(H55_RES)+1] = H55_KEY
      if H55_Getn(H55_RES) >= 10 then return H55_RES end
    end
...
function H55_Resolve(domain, token)
  if type(token) == 'number' then return token end
  H55_D, H55_T = H55_ParseToken(token)
  if H55_D == 'HERO' or (domain == 'HERO' and H55_D == nil) then
    ...
    if H55_IDS and H55_IDS[1] then
      if H55_Getn(H55_IDS) > 1 then error('Ambiguous hero skill tail '..H55_T..' matches multiple IDs; pass full const or domain prefix') end
      return H55_IDS[1]
    end
    H55_SUG = H55_Suggest('HERO', H55_T)
    error('Unknown hero skill/perk tail: '..H55_T..' ; suggestions: '..H55_Join(H55_SUG, ', '))
  elseif H55_D == 'ABILITY' or domain == 'ABILITY' then
    ...
    if H55_IDS and H55_IDS[1] then
      if H55_Getn(H55_IDS) > 1 then error('Ambiguous ability tail '..H55_T) end
      return H55_IDS[1]
    end
    H55_SUG = H55_Suggest('ABILITY', H55_T)
    error('Unknown ability tail: '..H55_T..' ; suggestions: '..H55_Join(H55_SUG, ', '))
  elseif H55_D == 'CREATURE' or domain == 'CREATURE' then
    ...
    if H55_IDS and H55_IDS[1] then
      if H55_Getn(H55_IDS) > 1 then error('Ambiguous creature tail '..H55_T) end
      return H55_IDS[1]
    end
    H55_SUG = H55_Suggest('CREATURE', H55_T)
    error('Unknown creature tail: '..H55_T..' ; suggestions: '..H55_Join(H55_SUG, ', '))
  else
    H55_OK, H55_ID = H55_PCall(H55_Resolve, 'HERO', H55_T)
    if H55_OK then return H55_ID end
    H55_OK, H55_ID = H55_PCall(H55_Resolve, 'ABILITY', H55_T)
    if H55_OK then return H55_ID end
    H55_OK, H55_ID = H55_PCall(H55_Resolve, 'CREATURE', H55_T)
    if H55_OK then return H55_ID end
    error('Cannot resolve token: '..H55_T)
  end
end

function H55_ConstById(domain, id)
  if domain == 'HERO' then
...
--[[ Usage: if H55_HeroHas('Agrael','PATHFINDING') then ... end ]]
function H55_HeroHas(heroName, token)
  H55_ID = H55_Resolve('HERO', token)
  return HasHeroSkill(heroName, H55_ID)
end

--[[ Usage: H55_AddHeroPerk('Agrael','PATHFINDING') ]]
function H55_AddHeroPerk(heroName, token)
  H55_ID = H55_Resolve('HERO', token)
  GiveHeroSkill(heroName, H55_ID)
end

--[[ Usage: H55_AddHeroSkill('Agrael','TACTICS') ]]
function H55_AddHeroSkill(heroName, token)
  H55_ID = H55_Resolve('HERO', token)
  GiveHeroSkill(heroName, H55_ID)
end

--[[ Usage: H55_EnsureHeroHas('Agrael','HERO_SKILL_TACTICS') ]]
function H55_EnsureHeroHas(heroName, token)
  H55_ID = H55_Resolve('HERO', token)
  if not HasHeroSkill(heroName, H55_ID) then GiveHeroSkill(heroName, H55_ID) end
end

--[[ Usage: H55_CreatureHasAbility('IMP','NO_RETALIATION') ]]
function H55_CreatureHasAbility(creatureToken, abilityToken)
  H55_CID = H55_Resolve('CREATURE', creatureToken)
  H55_AID = H55_Resolve('ABILITY', abilityToken)
...
-- ============================================================================
--  SKILL ⇢ PERKS TABLE  (flat “first column = skill, others = perks”)
--  Injected: 2025-08-15 18:53:37 UTC
-- ============================================================================
--[[
  H55_SKILL_PERK_TABLE is a flat, author-friendly description to avoid typos.
  Each row: first item is a HERO SKILL; the following items are PERKS of that skill.

  You can mix constants ('HERO_SKILL_*'/'HERO_SKILL_*' perk constants),
  numeric ids, or tails like 'TACTICS' / 'ATTACK'. Resolver makes it robust.
  Unknown tokens are ignored (no crash). Keep everything in this single file.

  EXAMPLE (minimal; extend for the full H55 tree):
    H55_SKILL_PERK_TABLE = {
      { 'ATTACK', 'TACTICS', 'ARCHERY', 'BATTLE_FRENZY' },
      { 'DEFENCE', 'PROTECTION', 'EVASION', 'LAST_STAND' },
      { 'LOGISTICS', 'PATHFINDING', 'SCOUTING', 'MARCH_OF_THE_MACHINES' },
      { 'LEADERSHIP', 'RECRUITMENT', 'DIVINE_GUIDANCE', 'BATTLE_COMMAND' },
      { 'LUCK', 'SOLDIER_S_LUCK', 'MAGIC_RESISTANCE', 'LUCKY_STRIKE' },
      { 'SORCERY', 'MAGIC_INSIGHT', 'ARCANE_TRAINING', 'CHANNELING' },
      { 'LIGHT_MAGIC', 'MASTER_OF_BLESSINGS', 'MASTER_OF_ABJURATION', 'MASTER_OF_WRATH' },
      { 'DARK_MAGIC', 'MASTER_OF_CURSES', 'MASTER_OF_MIND', 'MASTER_OF_SICKNESS' },
      { 'DESTRUCTIVE_MAGIC', 'MASTER_OF_FIRE', 'MASTER_OF_ICE', 'MASTER_OF_LIGHTNINGS' },
      { 'SUMMONING_MAGIC', 'MASTER_OF_CREATURES', 'MASTER_OF_QUAKES', 'MASTER_OF_CONJURATION' },
      { 'WAR_MACHINES', 'FIRST_AID', 'BALLISTA', 'CATAPULT' },
      { 'TRAINING', 'RETALIATION_STRIKE', 'STAMINA', 'BATTLE_COMMAND' },
      { 'AVENGER', 'FAVOURITE_ENEMY', 'ELVEN_LUCK', 'IMBUE_ARROW' },
      { 'ARTIFICIER', 'SPOILS_OF_WAR', 'MARCH_OF_THE_GOLEMS', 'ACADEMY_AWARD' },
      { 'SHATTER_DARK_MAGIC', 'CORRUPT_DARK', 'WEAKENING_STRIKE', 'DARK_REVELATION' },
      { 'SHATTER_LIGHT_MAGIC', 'CORRUPT_LIGHT', 'SOIL_BURN', 'LIGHT_REVELATION' },
      { 'SHATTER_DESTRUCTIVE_MAGIC', 'CORRUPT_DESTRUCTIVE', 'WEAKENING_STRIKE', 'ETERNAL_FLAME' },
      { 'SHATTER_SUMMONING_MAGIC', 'CORRUPT_SUMMONING', 'SOIL_BURN', 'CHAOTIC_SPELLS' },
    }

  NOTE: The names above are illustrative and may not match your exact H55 build.
        To finalize, replace with your authoritative pairs (see types.xml).
]]
H55_SKILL_PERK_TABLE = H55_SKILL_PERK_TABLE or { }

-- Runtime index: skillId -> array of perkIds
H55_SKILL_TO_PERKS = H55_SKILL_TO_PERKS or { }

-- Build the runtime index once
function H55_BuildSkillPerkIndex()
  H55_SKILL_TO_PERKS = { }
  H55_R = 1
  while H55_SKILL_PERK_TABLE[H55_R] do
    H55_ROW = H55_SKILL_PERK_TABLE[H55_R]
    H55_R = H55_R + 1
    if H55_ROW and H55_ROW[1] then
      H55_SID = H55_TryResolve('HERO', H55_ROW[1])
      if H55_SID then
        if H55_SKILL_TO_PERKS[H55_SID] == nil then H55_SKILL_TO_PERKS[H55_SID] = { } end
        H55_I = 2
        while H55_ROW[H55_I] do
          H55_PID = H55_TryResolve('HERO', H55_ROW[H55_I])
          if H55_PID then
            -- avoid duplicates
            if H55_SKILL_TO_PERKS[H55_SID][H55_PID] == nil then
              H55_SKILL_TO_PERKS[H55_SID][H55_PID] = true
            end
          end
          H55_I = H55_I + 1
        end
...
-- Ensure built on load
H55_BuildSkillPerkIndex()

-- Helpers
--[[ Usage: if H55_HeroHasPerk('Agrael','TACTICS') then ... end ]]
function H55_HeroHasPerk(heroName, perkToken)
  H55_PID = H55_TryResolve('HERO', perkToken)
  if H55_PID == nil then return false end
  return HasHeroSkill(heroName, H55_PID)
end

--[[ Usage: H55_AddPerkToHero('Agrael','TACTICS') ]]
function H55_AddPerkToHero(heroName, perkToken)
  H55_PID = H55_TryResolve('HERO', perkToken)
  if H55_PID == nil then
    print('[H55] Unknown perk token: '..tostring(perkToken))
    return
  end
  H55_SafeCall('GiveHeroSkill', GiveHeroSkill, heroName, H55_PID)
end

--[[ Usage: H55_AddSkillAndPerks('Agrael','ATTACK') ]]
function H55_AddSkillAndPerks(heroName, skillToken)
  H55_SID = H55_TryResolve('HERO', skillToken)
  if H55_SID == nil then
    print('[H55] Unknown skill token: '..tostring(skillToken))
    return
  end
  -- learn the base skill first
  if not HasHeroSkill(heroName, H55_SID) then
    H55_SafeCall('GiveHeroSkill', GiveHeroSkill, heroName, H55_SID)
  end
  -- then perks from the index, if any
  H55_PERKS = H55_SKILL_TO_PERKS[H55_SID]
  if type(H55_PERKS) == 'table' then
    H55_K = next(H55_PERKS, nil)
    while H55_K do
      if not HasHeroSkill(heroName, H55_K) then
        H55_SafeCall('GiveHeroSkill', GiveHeroSkill, heroName, H55_K)
      end
      H55_K = next(H55_PERKS, H55_K)
    end
  end
end

--[[ Usage: if H55_IsPerkOfSkill('TACTICS','ATTACK') then ... end ]]
function H55_IsPerkOfSkill(perkToken, skillToken)
  H55_SID = H55_TryResolve('HERO', skillToken)
  H55_PID = H55_TryResolve('HERO', perkToken)
  if H55_SID == nil or H55_PID == nil then return false end
  H55_PERKS = H55_SKILL_TO_PERKS[H55_SID]
  if type(H55_PERKS) ~= 'table' then return false end
  return H55_PERKS[H55_PID] == true
end


-- ============================================================================
--  SAFE SPELL CASTING (battle & adventure) + simple lore checks
--  Injected: 2025-08-15 18:53:37 UTC
-- ============================================================================
-- Optional ENUM.SPELL lookup (lightweight; fill as needed)
SPELL = SPELL or {}
SPELL_BY_CONST = SPELL_BY_CONST or {}
SPELL_BY_TAIL  = SPELL_BY_TAIL  or {}

-- Register helper (id is optional if you only use constants elsewhere)
function H55_RegisterSpell(constName, id)
  if constName == nil then return end
  SPELL[constName] = { id = id, name = constName, const = constName }
  SPELL_BY_CONST[constName] = id
  -- tail map
  H55_TAIL = string.gsub(constName, '^SPELL_', '')
  if H55_TAIL and H55_TAIL ~= '' then
    if SPELL_BY_TAIL[H55_TAIL] == nil then SPELL_BY_TAIL[H55_TAIL] = {} end
    table.insert(SPELL_BY_TAIL[H55_TAIL], id)
  end
end

-- Minimal known set (extend as needed)
H55_RegisterSpell('SPELL_RESURRECTION', nil)
H55_RegisterSpell('SPELL_HEAL', nil)
H55_RegisterSpell('SPELL_RAISE_DEAD', nil)

-- Resolve a spell similarly to H55_Resolve
function H55_ResolveSpell(token)
  if type(token) == 'number' then return token end
  H55_T = token
  if string.sub(H55_T, 1, 6) ~= 'SPELL_' then H55_T = 'SPELL_'..H55_T end
  if SPELL_BY_CONST[H55_T] then return SPELL_BY_CONST[H55_T] end
  H55_IDS = SPELL_BY_TAIL[string.gsub(H55_T, '^SPELL_', '')]
  if H55_IDS and H55_IDS[1] then
    if H55_IDS[2] ~= nil then error('Ambiguous spell tail: '..H55_T) end
    return H55_IDS[1]
  end
  error('Unknown spell: '..token)
end

-- Build an UNDREAD set (combines ability map + explicit creature list)
H55_UNDEAD_SET = H55_UNDEAD_SET or {}

function H55_AddUndead(creatureToken)
  H55_CID = H55_TryResolve('CREATURE', creatureToken)
  if H55_CID then H55_UNDEAD_SET[H55_CID] = true end
end

-- Populate common Necropolis lineup (skip silently if not present in the build)
H55_AddUndead('SKELETON'); H55_AddUndead('SKELETON_ARCHER'); H55_AddUndead('SKELETON_WARRIOR')
H55_AddUndead('ZOMBIE');   H55_AddUndead('WALKING_DEAD');   H55_AddUndead('PLAGUE_ZOMBIE')
H55_AddUndead('GHOST');    H55_AddUndead('SPECTRE')
H55_AddUndead('VAMPIRE');  H55_AddUndead('VAMPIRE_LORD');   H55_AddUndead('VAMPIRE_PRINCE')
H55_AddUndead('LICH');     H55_AddUndead('ARCHLICH');       H55_AddUndead('LICH_MASTER')
H55_AddUndead('WIGHT');    H55_AddUndead('WRAITH');         H55_AddUndead('BANSHEE')
H55_AddUndead('GHOUL');    H55_AddUndead('POLTERGEIST')

function H55_IsUndeadCreature(creatureToken)
  H55_CID = H55_TryResolve('CREATURE', creatureToken)
  if H55_CID == nil then return false end
  -- prefer explicit ability map if provided by ACTUAL_DATA
  if H55_CreatureHasAbility(H55_CID, 'UNDEAD') then return true end
  return H55_UNDEAD_SET[H55_CID] == true
end

-- Simple lore guard: disallow RESURRECTION/HEAL on undead; allow RAISE_DEAD
function H55_IsIllegalSpellOnUndead(spellToken, targetCreatureToken)
  H55_SID = H55_TryResolve('SPELL', spellToken) or H55_TryResolve('SPELL', 'RESURRECTION') -- normalize
  if H55_IsUndeadCreature(targetCreatureToken) then
    H55_NAME = tostring(spellToken)
    if H55_NAME == 'SPELL_RESURRECTION' or H55_NAME == 'RESURRECTION' or H55_NAME == 'HEAL' or H55_NAME == 'SPELL_HEAL' then
      return true
    end
  end
  return false
...
--[[
  H55_CombatCastSpellSafe(caster, spellToken, target)
  Tries several known engine entry points; refuses unsafe targets, never crashes.

  target can be:
    * creature token/id (preferred)
    * { x = <tileX>, y = <tileY> } for point spells

  Examples:
    -- Resurrection on undead creature → prints exception, no crash
    H55_CombatCastSpellSafe('Agrael', 'RESURRECTION', 'VAMPIRE_LORD')

    -- Raise Dead on undead creature → allowed
    H55_CombatCastSpellSafe('Agrael', 'RAISE_DEAD', 'SKELETON')

    -- Tile-targeted example
    H55_CombatCastSpellSafe('Agrael', 'FIREBALL', { x=10, y=9 })
]]
function H55_CombatCastSpellSafe(caster, spellToken, target)
  H55_SPELL_ID = H55_TryResolve('SPELL', spellToken) or spellToken
  if H55_SPELL_ID == nil then
    print('[H55] Unknown spell: '..tostring(spellToken))
    return false
  end

  -- Determine if target is a creature id
  H55_TCRE = nil
  if type(target) == 'table' then
    -- tile form {x=,y=} supported; nothing to check
  else
    H55_TCRE = H55_TryResolve('CREATURE', target)
  end

  if H55_TCRE and H55_IsIllegalSpellOnUndead(spellToken, H55_TCRE) then
    print('[H55][SAFE] Refusing to cast '..tostring(spellToken)..' on undead target '..tostring(target)..'.')
    return false
  end

  -- Try known battle-cast entry points in order
  H55_OK = false
  if CombatCastSpell ~= nil then
    H55_OK = H55_PCall(CombatCastSpell, caster, H55_SPELL_ID, target)[1]
    if H55_OK then return true end
  end
  if HeroCastSpell ~= nil then
    H55_OK = H55_PCall(HeroCastSpell, caster, H55_SPELL_ID, target)[1]
    if H55_OK then return true end
  end
  if StartCombatSpell ~= nil then
    H55_OK = H55_PCall(StartCombatSpell, caster, H55_SPELL_ID, target)[1]
    if H55_OK then return true end
  end

  print('[H55][SAFE] No known cast function succeeded for '..tostring(spellToken)..'.')
  return false
end


-- ============================================================================
--  DATE & GROWTH UTILITIES
--  Injected: 2025-08-15 18:53:37 UTC
-- ============================================================================

--[[
  H55_GetWeekNumber() -> 1..N
  Week 1 means days 1..7, week 2 means 8..14, etc.
]]
function H55_GetWeekNumber()
  H55_DAY = GetDate(ABSOLUTE_DAY)
  if H55_DAY == nil then return 1 end
  H55_W = math.floor((H55_DAY - 1) / 7) + 1
  if H55_W < 1 then H55_W = 1 end
  return H55_W
end

-- Base growth (per creature id). Fill from your build to get exact numbers.
-- Example: CREATURE_GROWTH[CREATURE_SKELETON] = 12
CREATURE_GROWTH = CREATURE_GROWTH or { }

--[[
  H55_GetBaseGrowth(creatureToken) -> number or 0
  Returns configured base growth for the creature, or 0 if unknown.
  Extend CREATURE_GROWTH in this file to cover your build.
]]
function H55_GetBaseGrowth(creatureToken)
  H55_CID = H55_TryResolve('CREATURE', creatureToken)
  if H55_CID == nil then return 0 end
  H55_V = CREATURE_GROWTH[H55_CID]
  if H55_V == nil then return 0 end
  return H55_V
end

--[[
  H55_CalcGrowth(creatureToken, opts) -> integer
  opts = { citadel = bool, castle = bool, asha = bool|number, multiplier = number }
  * citadel: +50% (if castle=false)
  * castle : +100% (over base; supersedes citadel)
  * asha   : true → +50%, or numeric additive fraction (e.g. 0.25)
  * multiplier: extra multiplier (e.g. external buffs); default 1.0
  Works for neutral stacks (no building bonuses) and town growth.

  Examples:
    -- Neutral growth for skeleton with a 20% event bonus
    local g = H55_CalcGrowth('SKELETON', { multiplier = 1.2 })

    -- Town growth with Citadel built
    local g = H55_CalcGrowth('ARCHER', { citadel = true })
]]
function H55_CalcGrowth(creatureToken, opts)
  if opts == nil then opts = {} end
  H55_CID = H55_TryResolve('CREATURE', creatureToken)
  if H55_CID == nil then return 0 end
  H55_BASE = H55_GetBaseGrowth(H55_CID)
  H55_MUL = 1.0
  if opts.castle then
    H55_MUL = 2.0
  elseif opts.citadel then
    H55_MUL = 1.5
  end
  if opts.asha then
    if type(opts.asha) == 'number' then
      H55_MUL = H55_MUL * (1.0 + opts.asha)
    else
      H55_MUL = H55_MUL * 1.5
    end
  end
  if opts.multiplier and opts.multiplier ~= 1 then
    H55_MUL = H55_MUL * opts.multiplier
  end
  return math.floor(H55_BASE * H55_MUL + 0.0001)
end


-- ============================================================================
--  ARMY HELPERS (first free slot)
--  Injected: 2025-08-15 18:53:37 UTC
-- ============================================================================

--[[
  H55_AddToFirstFreeSlot(heroName, creatureToken, amount)
  Uses engine's AddHeroCreatures which merges to existing stack or first free slot.
  Always safe-resolved to avoid typos.
  Example:
    H55_AddToFirstFreeSlot('Agrael', 'IMP', 15)
]]
function H55_AddToFirstFreeSlot(heroName, creatureToken, amount)
  H55_CID = H55_TryResolve('CREATURE', creatureToken)
  if H55_CID == nil then
    print('[H55] Unknown creature token: '..tostring(creatureToken))
    return
  end
  H55_SafeCall('AddHeroCreatures', AddHeroCreatures, heroName, H55_CID, amount or 0)
end

--[[
  H55_AddArmyList(heroName, list)
  list = { {creature, count}, {creature, count}, ... }
  Example:
    H55_AddArmyList('Agrael', {
      {'IMP', 60}, {'DEMON', 30}, {'HELL_HOUND', 20}
    })
]]
function H55_AddArmyList(heroName, list)
  if type(list) ~= 'table' then return end
  H55_I = 1
  while list[H55_I] do
    H55_ROW = list[H55_I]
    if H55_ROW and H55_ROW[1] and H55_ROW[2] then
      H55_AddToFirstFreeSlot(heroName, H55_ROW[1], H55_ROW[2])
    end
    H55_I = H55_I + 1
  end
end

-- End of injected framework
