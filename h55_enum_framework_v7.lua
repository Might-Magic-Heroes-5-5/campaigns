...
===============================================================================
 ...
 Generated: 2025-08-15 22:43:29 UTC
 This file is derived from v6 and includes the following **important** updates:

  • **Spell safety (Blind & other mind-control)**:
      - New immunity checks for Blind/Frenzy/Puppet Master/Confusion.
      - Accounts for *Undead*, *Mechanical*, *Elemental*, explicit
        **IMMUNITY_TO_BLIND**, and temporary **VAMPIRISM** (when detectable).
  • **pcall table-mode (Lua 3.8/4)**:
      - New `_H55_PCallT(...)` that returns the *raw* table from `pcall`.
        `_H55_SafeCallT(name, ...)` mirrors this behavior.
      - Kept `_H55_PCall(...)` for back-compat (expands first 10 returns).
  • **Growth data**:
      - Added tier defaults + many creature overrides to `H55_BASE_GROWTH`.
        Falls back to tier defaults when a creature override is missing.
  • **UX & consistency**:
      - All public helpers keep `_H55_` prefix; lowercase aliases retained.
      - All actions support `showText` to display flying signs.
  • **Docs**:
      - Each new function has examples and clear notes for maintainers.

 NOTE: This framework targets MMH55’s Lua 4 VM variant where `pcall` returns
       an *indexed table*. See `_H55_PCallT` below when you want to work with
       that table directly.
...
-- =====================================================================
-- pcall helpers — table-mode (Lua 3.8/4)
-- =====================================================================
--[[
_H55_PCallT(f, ...) -> table_like
Returns the *raw* table returned by `pcall` in this VM (index 1..N),
so callers can access fields directly (e.g. r[1] is ok flag, r[2] is ret1).
This keeps allocations minimal and avoids tuple expansion.
Examples:
  local r = _H55_PCallT(HasHeroSkill, 'Agrael', HERO_SKILL_OFFENCE)
  if r and r[1] then print('ok:', r[2]) end

Back-compat:
  `_H55_PCall` (expanding version) still exists and continues to work.
]]
function _H55_PCallT(f, a1,a2,a3,a4,a5,a6,a7,a8,a9)
  if f == nil then return { false, 'nil function' } end
  return pcall(f, a1,a2,a3,a4,a5,a6,a7,a8,a9)
end

--[[
_H55_SafeCallT(name, ...) -> table_like
Looks up a global by name and calls it via `_H55_PCallT`. Useful when the
function may not exist in a given context (campaign vs. combat vs. map scripts).
Examples:
  local r = _H55_SafeCallT('IsCombatUnitAffectedBySpell', stackId, SPELL.VAMPIRISM)
  if r and r[1] and r[2] then ... end
]]
function _H55_SafeCallT(name, a1,a2,a3,a4,a5,a6,a7,a8,a9)
  H55_F = _G and _G[name] or nil
  if H55_F == nil and type(getglobal) == 'function' then
    H55_F = getglobal(name)
  end
  if type(H55_F) ~= 'function' then
    return { false, 'missing function: '..tostring(name) }
  end
  return _H55_PCallT(H55_F, a1,a2,a3,a4,a5,a6,a7,a8,a9)
end
--[[
===============================================================================
 MMH55 ENUM/RESOLVER FRAMEWORK — v6 (Lua 4 / Lua 3.8 compatible, NO 'local')
...
function _H55_SafeCastSpell(casterHero, spellIdOrToken, targetSpec1, targetSpec2, showText)
  -- Heuristic "raise/resurrect" and "mind-control" guards
  H55_OKA, H55_REASON = _H55_SpellAffectsTarget(spellIdOrToken, targetSpec1)
  if not H55_OKA then
    if showText then _H55_Fly(casterHero, 'SPELL BLOCKED '..tostring(spellIdOrToken)..' ('..tostring(H55_REASON)..')') end
    print('_H55_SafeCastSpell: blocked cast of '..tostring(spellIdOrToken)..' due to '..tostring(H55_REASON))
    return false, H55_REASON
  end

  -- Resurrection vs. Undead guard (Resurrection should not target Undead)
  H55_STOK = spellIdOrToken
  if type(H55_STOK) == 'string' then H55_STOK = _H55_ToUpper(H55_STOK) end
  H55_IS_RES = (type(H55_STOK) == 'string') and (string.find(H55_STOK, 'RESUR', 1, true) ~= nil)
  if H55_IS_RES and targetSpec1 ~= nil and _H55_IsUndeadCreature(targetSpec1) then
    if showText then _H55_Fly(casterHero, 'SPELL BLOCKED '..tostring(spellIdOrToken)..' (undead target)') end
    print('_H55_SafeCastSpell: deny Resurrection on undead target')
    return false, 'undead target'
  end
  ...
  H55_R = _H55_PCallT(CombatCastSpell, casterHero, spellIdOrToken, targetSpec1, targetSpec2)
  if H55_R and H55_R[1] then
    if showText then _H55_Fly(casterHero, 'SPELL CAST '..tostring(spellIdOrToken)) end
    return true, H55_R[2]
  end
  print('_H55_SafeCastSpell: engine rejected cast: '..tostring(H55_R and H55_R[2]))
  return false, H55_R and H55_R[2]
end
...
H55_BASE_GROWTH = H55_BASE_GROWTH or {
  -- ==== Vanilla-like baseline overrides (adjust per MMH55 balance) ====
  -- Tier 1
  ['PEASANT']=25,   ['SKELETON']=12,  ['IMP']=18,      ['GREMLIN']=18,   ['SPRITE']=16,  ['GOBLIN']=15,
  -- Tier 2
  ['ARCHER']=10,    ['FAMILIAR']=18,  ['STONE_GARGOYLE']=14, ['BLOOD_MAIDEN']=12, ['DRUID']=7,   ['WOLF_RAIDER']=7,
  -- Tier 3
  ['FOOTMAN']=7,    ['CERBERUS']=7,   ['GOLEM']=7,          ['BLOOD_FURY']=10,   ['HUNTER']=7,  ['SPEARWIELDER']=9,
  -- Tier 4
  ['PRIEST']=4,     ['SUCCUBUS']=5,   ['ARCHLICH']=3,       ['RAKSHASA']=3,      ['WITCH']=4,   ['SKIRMISHER']=5,
  -- Tier 5
  ['CAVALIER']=3,   ['HELL_HOUND']=3, ['WRAITH']=2,         ['DJINN']=3,         ['TREANT']=2,  ['BERSERKER']=3,
  -- Tier 6
  ['PALADIN']=2,    ['PIT_LORD']=2,   ['VAMPIRE_LORD']=2,   ['MAGI_ARCHMAGE']=2, ['UNICORN']=2, ['THANE']=2,
  -- Tier 7
  ['ANGEL']=2,      ['DEVIL']=1,      ['BONE_DRAGON']=1,    ['TITAN']=1,         ['BLACK_DRAGON']=1, ['GREEN_DRAGON']=1,
}
...
-- Default tier growths used when a creature has no explicit override above.
-- These are safe, conservative values close to vanilla ToE. Tweak for H55.
H55_DEFAULT_GROWTH_BY_TIER = H55_DEFAULT_GROWTH_BY_TIER or {
  [1]=20, [2]=12, [3]=9, [4]=7, [5]=4, [6]=3, [7]=2
}

-- Optional helper to guess tier from a creature id using FACTION_UNITS map.
function _H55__GuessTier(creatureToken)
  if type(FACTION_UNITS) ~= 'table' then return nil end
  H55_CID = _H55_Resolve('CREATURE', creatureToken)
  H55_TOWN = 1
  while FACTION_UNITS[H55_TOWN] do
    H55_TIERS = FACTION_UNITS[H55_TOWN]
    H55_TR = 1
    while H55_TIERS[H55_TR] do
      H55_G = FACTION_UNITS[H55_TOWN][H55_TR]
      if H55_G and (H55_G[1] == H55_CID or H55_G[2] == H55_CID or H55_G[3] == H55_CID) then
        return H55_TR
      end
      H55_TR = H55_TR + 1
    end
...
-- Re-implement ComputeGrowth to use overrides + tier fallback.
-- Signature preserved.
function _H55_ComputeGrowth(creatureToken, hasCitadel, hasCastle, hasAsha, multiplier)
  H55_CNAME = _H55__CreatureConst(creatureToken)
  H55_BASE  = H55_BASE_GROWTH[H55_CNAME]
  if H55_BASE == nil then
    H55_TIER = _H55__GuessTier(creatureToken) or 3
    H55_BASE = H55_DEFAULT_GROWTH_BY_TIER[H55_TIER] or 5
  end
  H55_M = multiplier or 1.0
  -- Citadel (+50%), Castle (+100%), Asha (+100%) — multiplicative on base.
  H55_GROW = H55_BASE
  if hasCitadel then H55_GROW = H55_GROW + (H55_BASE * 0.5) end
  if hasCastle  then H55_GROW = H55_GROW + (H55_BASE * 1.0) end
  if hasAsha    then H55_GROW = H55_GROW + (H55_BASE * 1.0) end
  H55_GROW = math.floor(H55_GROW * H55_M + 0.0001)
  if H55_GROW < 0 then H55_GROW = 0 end
  return H55_GROW
end

-- =====================================================================
-- SPELL IMMUNITY HELPERS (Blind & Mind-control)
-- =====================================================================
-- Ability constants we care about (works with dot-notation too).
-- If ABILITY table is not built yet in your stack, these lines are harmless.
H55__A_UNDEAD              = ABILITY and (ABILITY.UNDEAD or ABILITY.UNDEAD and ABILITY.UNDEAD.id) or 'UNDEAD'
H55__A_MECHANICAL          = ABILITY and (ABILITY.MECHANICAL or ABILITY.MECHANICAL and ABILITY.MECHANICAL.id) or 'MECHANICAL'
H55__A_ELEMENTAL           = ABILITY and (ABILITY.ELEMENTAL or ABILITY.ELEMENTAL and ABILITY.ELEMENTAL.id) or 'ELEMENTAL'
H55__A_IMMUNITY_TO_BLIND   = ABILITY and (ABILITY.IMMUNITY_TO_BLIND or ABILITY.IMMUNITY_TO_BLIND and ABILITY.IMMUNITY_TO_BLIND.id) or 'IMMUNITY_TO_BLIND'
H55__A_IMMUNITY_TO_MIND    = ABILITY and (ABILITY.IMMUNITY_TO_MIND_CONTROL or ABILITY.IMMUNITY_TO_MIND_CONTROL and ABILITY.IMMUNITY_TO_MIND_CONTROL.id) or 'IMMUNITY_TO_MIND_CONTROL'

-- Known families used when CREATURE_ABILITY_MAP is not fully populated.
H55__KNOWN_UNDEAD = H55__KNOWN_UNDEAD or {
  SKELETON=true, SKELETON_ARCHER=true, SKELETON_WARRIOR=true, SKELETON_ARCHER_ALT=true,
  ZOMBIE=true, DISEASE_ZOMBIE=true, PLAGUE_ZOMBIE=true,
  GHOST=true, SPECTRE=true, WRAITH=true, WIGHT=true,
  VAMPIRE=true, VAMPIRE_LORD=true, NOSFERATU=true,
  LICH=true, ARCHLICH=true, LICH_MASTER=true,
  BONE_DRAGON=true, SHADOW_DRAGON_UNDEAD=true, GHOST_DRAGON=true,
  DEATH_KNIGHT=true, BLACK_KNIGHT=true,
}

H55__KNOWN_MECHANICAL = H55__KNOWN_MECHANICAL or {
  GOLEM=true, IRON_GOLEM=true, STEEL_GOLEM=true, MAGNETIC_GOLEM=true,
  GARGOYLE=true, OBSIDIAN_GARGOYLE=true, STONE_GARGOYLE=true,
}

H55__KNOWN_ELEMENTAL = H55__KNOWN_ELEMENTAL or {
  FIRE_ELEMENTAL=true, WATER_ELEMENTAL=true, AIR_ELEMENTAL=true, EARTH_ELEMENTAL=true,
  STORM_ELEMENTAL=true, MAGMA_ELEMENTAL=true, ICE_ELEMENTAL=true, ENERGY_ELEMENTAL=true,
  PHOENIX=true, FIREBIRD=true
}

-- Heuristic: resolve a creature token/id to a constant string for set lookup.
function _H55__CreatureConst(creatureToken)
  H55_ID = _H55_Resolve and _H55_Resolve('CREATURE', creatureToken) or creatureToken
  H55_NAME = _H55_ConstById and _H55_ConstById('CREATURE', H55_ID) or tostring(creatureToken)
  return _H55_ToUpper(H55_NAME)
end

-- Returns true if creature is Undead based on ability map or known families.
function _H55_IsUndeadCreature(creatureToken)
  H55_C = _H55__CreatureConst(creatureToken)
  -- Check explicit ability map if present
  if type(CREATURE_ABILITY_MAP) == 'table' then
    H55_CID = _H55_Resolve('CREATURE', creatureToken)
    H55_SET = CREATURE_ABILITY_MAP[H55_CID]
    if type(H55_SET) == 'table' then
      -- linear scan (Lua 4 style)
      H55_I = 1
      while H55_SET[H55_I] do
        if H55_SET[H55_I] == H55__A_UNDEAD or H55_SET[H55_I] == 'UNDEAD' then return true end
        H55_I = H55_I + 1
      end
      if H55_SET[H55__A_UNDEAD] ~= nil then return true end
    end
  end
  -- Fallback to family set (by const)
  return H55__KNOWN_UNDEAD[H55_C] == true
end

-- Returns true if creature is Mechanical or Elemental family (mind immune).
function _H55_IsConstructOrElemental(creatureToken)
  H55_C = _H55__CreatureConst(creatureToken)
  if type(CREATURE_ABILITY_MAP) == 'table' then
    H55_CID = _H55_Resolve('CREATURE', creatureToken)
    H55_SET = CREATURE_ABILITY_MAP[H55_CID]
    if type(H55_SET) == 'table' then
      ...
      while H55_SET[H55_I] do
        H55_A = H55_SET[H55_I]
        if H55_A == H55__A_MECHANICAL or H55_A == 'MECHANICAL' or H55_A == H55__A_ELEMENTAL or H55_A == 'ELEMENTAL' then
          return true
        end
        H55_I = H55_I + 1
      end
      if H55_SET[H55__A_MECHANICAL] ~= nil or H55_SET[H55__A_ELEMENTAL] ~= nil then return true end
    end
  end
  return (H55__KNOWN_MECHANICAL[H55_C] == true) or (H55__KNOWN_ELEMENTAL[H55_C] == true)
end

-- Generic "mind-control immunity" (undead OR mechanical/elemental OR explicit mind immunity perk).
function _H55_IsMindImmune(creatureToken)
  if _H55_IsUndeadCreature(creatureToken) then return true end
  if _H55_IsConstructOrElemental(creatureToken) then return true end
  -- Check explicit mind-immunity ability from ability map
  if type(CREATURE_ABILITY_MAP) == 'table' then
    H55_CID = _H55_Resolve('CREATURE', creatureToken)
    H55_SET = CREATURE_ABILITY_MAP[H55_CID]
    if type(H55_SET) == 'table' then
      H55_I = 1
      while H55_SET[H55_I] do
        if H55_SET[H55_I] == H55__A_IMMUNITY_TO_MIND or H55_SET[H55_I] == 'IMMUNITY_TO_MIND_CONTROL' then return true end
        H55_I = H55_I + 1
      end
      if H55_SET[H55__A_IMMUNITY_TO_MIND] ~= nil then return true end
    end
  end
  return false
end

-- Blind-specific immunity (explicit ability OR generic mind immunity OR VAMPIRISM buff).
function _H55_IsBlindImmune(creatureTokenOrStack)
  -- If the call passes a combat stack id, try to derive its creature type
  H55_CTOKEN = creatureTokenOrStack
  if type(creatureTokenOrStack) == 'number' then
    -- Attempt several engine functions to resolve stack -> creature type
    H55_R = _H55_SafeCallT('GetCombatUnitType', creatureTokenOrStack)
    if H55_R and H55_R[1] and H55_R[2] then
      H55_CTOKEN = H55_R[2]
    end
  end

  -- Check explicit "immunity to blind" ability
  if type(CREATURE_ABILITY_MAP) == 'table' then
    H55_CID = _H55_Resolve('CREATURE', H55_CTOKEN)
    H55_SET = CREATURE_ABILITY_MAP[H55_CID]
    if type(H55_SET) == 'table' then
      H55_I = 1
      while H55_SET[H55_I] do
        if H55_SET[H55_I] == H55__A_IMMUNITY_TO_BLIND or H55_SET[H55_I] == 'IMMUNITY_TO_BLIND' then return true end
        H55_I = H55_I + 1
      end
      if H55_SET[H55__A_IMMUNITY_TO_BLIND] ~= nil then return true end
    end
  end

  -- Check generic mind-immunity (covers undead/mechanical/elemental)
  if _H55_IsMindImmune(H55_CTOKEN) then return true end

  -- Temporary "Vampirism" buff makes a unit effectively Undead -> mind immune
  -- We try several probable API names; if none are present we simply skip.
  H55_V = _H55_SafeCallT('IsCombatUnitAffectedBySpell', creatureTokenOrStack, (SPELL and (SPELL.VAMPIRISM or SPELL and SPELL.VAMPIRISM and SPELL.VAMPIRISM.id)) or 'VAMPIRISM')
  if H55_V and H55_V[1] and H55_V[2] then return true end

  return false
end

-- Determines if a given spell *should* affect target, guarding common pitfalls.
-- Currently protects: BLIND, FRENZY, PUPPET MASTER, CONFUSION vs. mind-immunity.
-- Returns: ok(bool), reason(string or nil)
function _H55_SpellAffectsTarget(spellIdOrToken, targetCreatureOrStack)
  -- Heuristic by spell token name (safe across versions)
  H55_STOK = spellIdOrToken
  if type(H55_STOK) == 'string' then H55_STOK = _H55_ToUpper(H55_STOK) end

  H55_IS_BLIND   = (type(H55_STOK) == 'string') and (string.find(H55_STOK, 'BLIND', 1, true) ~= nil)
  H55_IS_FRENZY  = (type(H55_STOK) == 'string') and (string.find(H55_STOK, 'FRENZ', 1, true) ~= nil)
  H55_IS_PUPPET  = (type(H55_STOK) == 'string') and (string.find(H55_STOK, 'PUPPET', 1, true) ~= nil)
  H55_IS_CONFUSE = (type(H55_STOK) == 'string') and (string.find(H55_STOK, 'CONFU', 1, true) ~= nil)

  if H55_IS_BLIND or H55_IS_FRENZY or H55_IS_PUPPET or H55_IS_CONFUSE then
    if _H55_IsBlindImmune(targetCreatureOrStack) then
      return false, 'mind-immune or blind-immune'
    end
  end

  -- Add more families here if needed (e.g., magic immunity/rune wards/etc.)
  return true, nil
end
