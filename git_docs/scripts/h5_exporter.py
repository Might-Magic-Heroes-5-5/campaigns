...
# -*- coding: utf-8 -*-
"""
HOMM5 Build Helper — exports:
  • CREATURES .xdb -> h55_enums_creatures.lua
  • SPELLS    .xdb -> h55_enums_spells.lua

Static helpers live in: git_docs/scripts/h55_enum_framework.lua
"""
...
def header(title: str) -> None: print("\n"+title+"\n"+("="*len(title)))
...
def write_one(base_dir: Path, rel_name: str, content: str, dry_run: bool) -> Path:
    ...
    if not dry_run:
        ...
        info(f"Dry-run; not writing {out_path}")
    return out_path

def output_base(out_dir: Path, layout: str) -> Path:
    if layout == "scripts":
        return out_dir / "scripts"
    return out_dir

def copy_framework(script_dir: Path, base_dir: Path) -> None:
    src = script_dir / "h55_enum_framework.lua"
    if not src.exists():
        warn(f"Static helper not found: {src} (skipping)")
        return
    content = src.read_text(encoding="utf-8")
    write_one(base_dir, "h55_enum_framework.lua", content, dry_run=False)

# ---------- xml helpers ----------
def read_xml(path: Path) -> Optional[ET.ElementTree]:
    try: return ET.parse(str(path))
...
def try_load_ids_from_framework(repo_root: Path) -> Tuple[Dict[str, int], Dict[str, int]]:
    ...
    for f in repo_root.rglob("h55_enum_framework*.lua"):
...
def types_xml_sanity(types_xml: Path) -> Tuple[str, int, int, int]:
    try:
        tree = ET.parse(str(types_xml)); root = tree.getroot()
    except Exception as e:
...
def parse_creature_xdb(xdb_path: Path) -> Tuple[str, List[str], int]:
    ...
    if tree is None: return enum_tail, [], 0
    root = tree.getroot()
    abilities: List[str] = []
...
def parse_spell_xdb(xdb_path: Path) -> Tuple[str, str]:
    enum_tail = slug_to_enum_tail(xdb_path.stem)
    tree = read_xml(xdb_path)
    if tree is None: return enum_tail, f"SPELL_{enum_tail}"
...
def lua_framework_bootstrap() -> str:
    # raw pcall allowed here for the first import only
    return r"""
-- Bootstrap: ensure framework is loaded once (raw pcall allowed only here).
if not _h55_doFile then
  function _h55_doFile(name, log_level)
    local p1 = "./scripts/"..tostring(name)
    local p2 = "./"..tostring(name)
    if doFile then
      local ok = pcall(doFile, p1); if ok then return {1} end
      ok = pcall(doFile, p2); if ok then return {1} end
    end
    return nil
  end
end
_h55_doFile("h55_enum_framework.lua", 1)
""".strip()+"\n"
...
def build_lua_creatures_dot(creatures_by_town: Dict[str, Dict[str, Dict]]) -> str:
    ...
    for town in sorted(creatures_by_town.keys()):
        ...
        for name in sorted(creatures_by_town[town].keys()):
            ...
            const = o['const']; cid = o['id']; baseg = o['BASE_GROWTH']
...
def export_creatures(repo_root: Path, local_dir: Path, types_xml: Path,
                     out_dir: Path, layout: str, dry_run: bool, strict: bool) -> None:
    header("creatures export")

    base_dir = output_base(out_dir, layout)
    ...
    for town, enum_tail, abilities, growth in creatures_tmp:
        ...
        cid = framework_cre_ids.get(const) or pdf_ids.get(const)
        if cid is None:
            if strict:
        ...
        creatures_by_town[town][enum_tail] = dict(
            const=const, id=cid, BASE_GROWTH=int(growth or 0), ABILITIES=abilities
        )

    ability_ids = framework_abi_ids or {a: 0 for _, _, A, _ in creatures_tmp for a in A}

    lua_chunks: List[str] = []
    lua_chunks.append(lua_framework_bootstrap())
    ...
    ensure_dir(base_dir)
    copy_framework(Path(__file__).resolve().parent, base_dir)

    info(f"Planned output: {base_dir / 'h55_enums_creatures.lua'}")
    write_one(base_dir, "h55_enums_creatures.lua", lua_text, dry_run=dry_run)
...
def export_spells(repo_root: Path, local_dir: Path, out_dir: Path, layout: str, dry_run: bool) -> None:
    ...
    info(f"Planned output: {base_dir / 'h55_enums_spells.lua'}")
    write_one(base_dir, "h55_enums_spells.lua", lua_text, dry_run=dry_run)
...
def choose_output_dir(local_dir: Path) -> Path:
    ...
    while True:
        ...
        if sel == "1":
            ...
            ensure_dir(out); info(f"Using output directory: {out}")
            return out
        if sel == "2":
...
def main() -> int:
    ...
    parser.add_argument("--layout", choices=["scripts","root"], default="scripts",
                        help="where to place outputs inside the chosen OUT_DIR (default: scripts)")
    ...
    while True:
        ...
        if sel in (1, 3):
            export_creatures(repo_root, local_dir, types_xml, out_dir, args.layout, args.dry_run, args.strict)
        if sel in (2, 3):
            export_spells(repo_root, local_dir, out_dir, args.layout, args.dry_run)
