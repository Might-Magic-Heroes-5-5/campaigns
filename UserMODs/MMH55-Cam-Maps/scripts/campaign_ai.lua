...
function _AI_MostImportantTarget(hero, ignore_threshold)
  local minIndex, minValue = 1, math.huge;
	for i, value in hero.weights do
    if value > ignore_threshold and value <= minValue then
      minIndex = i;
			minValue = value;
			_AI_print(1, ignore_threshold .. " | " .. i .. ": " .. value .. " | " .. hero.targets[i]);
    end
	end
...
function _AI_SetHeroTarget(name, target, ttype)
...
function _AI_FindHeroTarget(name, hero, tries_left, threshold)
  ...
  local choice = _AI_SetHeroTarget(name, hero.targets[idx], hero.types[idx])
...
function _AI_AddHeroTargets(name, hero, player, list, ttype, ai)
  _AI_print(2, "Adding targets " .. ttype .. " for hero " .. name)
  for _, item in list do
    owner_status = pcall(GetObjectOwner, item);
    place_status = pcall(GetObjectPosition, item);
    if owner_status ~= nil and place_status ~= nil then
      ...
      if owner ~= player and ai.enemies[owner].is_enemy == 1 then
  	    local x     = place_status[1];
        local y     = place_status[2];
        local z     = place_status[3];
        local cost  = 0;
        local p_cost = pcall(CalcHeroMoveCost,name,x,y,z);
        if p_cost[1] < 0 then                               -- CalcHeroMoveCost fails when hero is in castles so using an estimate of the distance to target
...
function _AI_UpdateTargetWeight(player)
  _AI_print(2, "updating weight for player " .. player)
