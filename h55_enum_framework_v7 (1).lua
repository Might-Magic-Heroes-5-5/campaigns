--[[
===============================================================================
 MMH55 ENUM/RESOLVER FRAMEWORK — v6 (Lua 4 / Lua 3.8 compatible, NO 'local')
 Generated: 2025-08-15 22:17:13 UTC
 Author(s): MMH55 team (framework scaffold), this iteration by Alexander L.
 Source of IDs: MMH55-Frame.pak\types.xml (skills/perks/abilities/creatures)

 WHAT'S NEW IN v6 (over v5):
   • **Dot-notation PERK table is now truly flat** (PERK.SNATCH etc.).
     We build PERK from HERO_PERK families so dot-notation always works.
   • **Uppercase API parity**: Added _H55_GivePerkSoft/_H55_GivePerkHard/
     _H55_RemovePerk/_H55_GiveSkill/_H55_UpgradeSkill wrappers (v5 had lowercase).
   • **Growth math cleanup**: fixed edge-case in _H55_ComputeGrowth and documented
     usage clearly; result is deterministic, integer, and stable.
   • **Consistent flying-text messages** in all helpers ("PERK GIVEN {...}",
     "CREATURES GIVEN {...}", etc.) for both debugging and UX feedback.
   • **Reserved IDs called out** explicitly in enum blocks to reduce mistakes.
   • Minor performance & safety polish (early-outs, table lookups, pcall guards).

 USAGE COOKBOOK (short reminders; see per-function blocks for more examples):
   -- Skills & levels (ultimates auto-refused in H55; optional fallback map):
   _H55_GiveSkill('Ylaya', SKILL.DARK_MAGIC, LEVEL.BASIC, true)
   _H55_UpgradeSkill('Ylaya', SKILL.DARK_MAGIC, LEVEL.EXPERT, true)
   _H55_SetHeroSkillLevel('Ylaya', SKILL.DARK_MAGIC, LEVEL.ADVANCED)

   -- Perks (soft/hard grant + removal):
   _H55_GivePerkSoft('Agrael', PERK.SNATCH, true)
   _H55_GivePerkHard('Vittorio', PERK.MENTORING, LEVEL.BASIC, true)
   _H55_RemovePerk('Agrael', PERK.SNATCH, true)

   -- Creatures (town/tier/grade DOT enums; first empty slot; safe remove/multiply):
   _H55_GiveCreaturesTo('Agrael', TOWN.HAVEN, TIER.ONE, GRADE.ONE, 40, true)
   _H55_MultiplyCreaturesOfHero('Agrael', TOWN.INFERNO, TIER.THREE, GRADE.UPGRADE_A, 2, true)
   _H55_RemoveCreature('Agrael', TOWN.HAVEN, TIER.FOUR, GRADE.UPGRADE_A, 10, true)
   _H55_RemoveCreatureTown('TownOfAshwood', TOWN.NECROPOLIS, TIER.TWO, GRADE.UPGRADE_A, 12, true)

   -- Spells (safe cast wrapper; protects against invalid/undead targets):
   _H55_SafeCastSpell('Zehir', 'RESURRECTION', allyUnitId)  -- will skip undead safely

   -- Week & growth:
   local week = _H55_GetWeekNumber()
   local peasants = _H55_ComputeGrowth('PEASANT', true, true, false, 1.0)

 NOTES
   • Lua 4 VM variant (MMH55): pcall returns a TABLE; use _H55_PCall wrapper.
   • No 'local' variables or local functions used in this file.
   • All public helpers start with _H55_ (lowercase aliases kept for back-compat).
===============================================================================
]]
--[[
===============================================================================
 MMH55 ENUM/RESOLVER FRAMEWORK — v4 (Lua 4 / Lua 3.8 compatible, NO 'local')
 Generated: 2025-08-15 21:27:22 UTC
 Author(s): MMH55 team (framework scaffold), this iteration by Alexander L.
 Source of IDs: MMH55-Frame.pak\types.xml (skills/perks/abilities/creatures)

 WHAT'S NEW IN v4 (over v3):
   • Big "cookbook" of usage examples right here at the top.
   • Skill *levels* supported via _H55_SetHeroSkillLevel/_H55_AddHeroSkillAtLevel.
     → Example: add EXPERT Dark Magic to hero 'Ylaya':
         _H55_AddHeroSkillAtLevel('Ylaya', 'DARK_MAGIC', 'EXPERT')
   • Ultimates are disabled in H55. All grant helpers now *refuse* ABSOLUTE_* 
     and optionally fallback if you configure H55_ULTIMATE_FALLBACK.
   • New queries: _H55_GetPerksOfSkill, _H55_GetSkillOfPerk, _H55_SkillHasPerk.
   • Expanded examples for every preserved function (see below).
   • File keeps the explicit "RESERVED IDs" notes for engine placeholders.

 IMPORTANT LUA NOTE FOR MMH55:
   - This environment uses a Lua flavor where pcall(...) returns an *indexed table*.
     Use _H55_PCall(...) in this file; it unwraps to (ok, ret1, ret2, ...).

 QUICK-START (DONT SKIP):
   1) Drop this file into your map/mod and `dofile('h55_enum_framework_v4.lua')` once.
   2) You can now use *tokens* (case-insensitive tails) or full const names, or IDs:
        _H55_AddHeroPerk('Agrael', 'Snatch')                -- tail token
        _H55_AddHeroPerk('Agrael', 'HERO_SKILL_SNATCH')     -- const string
        _H55_AddHeroPerk('Agrael', 149)                     -- numeric id (if known)
   3) Grant "Expert Dark Magic" (skill with mastery level):
        _H55_AddHeroSkillAtLevel('Ylaya', 'Dark_Magic', 'Expert')
        -- or, if you only need to *set* level (assumes skill already given):
        _H55_SetHeroSkillLevel('Ylaya', 'HERO_SKILL_DARK_MAGIC', 'EXPERT')

 ULTIMATES (ABSOLUTE_*):
   - Heroes 5.5 does NOT support ultimate skills. This framework blocks them safely.
   - If your script ever tries to grant an ultimate, you'll see a warning and *no crash*.
   - Optional fallback: map an ABSOLUTE_* id to a regular perk or base skill:
       H55_ULTIMATE_FALLBACK = H55_ULTIMATE_FALLBACK or (an empty table)
       H55_ULTIMATE_FALLBACK[HERO_SKILL_ABSOLUTE_GATING] = HERO_SKILL_CRITICAL_GATING
       -- From there, _H55_AddHeroPerk('Agrael','ABSOLUTE_GATING') will give CRITICAL_GATING.

 CREATURES & TIERS (author-friendly):
   - Give stacks by town/tier/grade using readable enums (internal to framework):
       give_by_tier('Agrael', TOWN.HAVEN, TIER.ONE,   GRADE.ONE,       40) -- Peasants
       give_by_tier('Agrael', 'Haven',   'Two',       'Upgrade_A',     12) -- Marksmen
       give_by_tier('Agrael', TOWN.HAVEN, TIER.FOUR,  GRADE.UPGRADE_A, 25) -- Royal Griffins
     Under the hood this maps to CREATURE_* IDs and uses first empty slot.

 SAFE SPELLS (no crashes on lore rules):
   - Use _H55_SafeCastSpell(...) instead of CombatCastSpell(...). It will skip
     dangerous casts (e.g., "raise/resurrect" onto undead) and print a message
     rather than crashing:
       _H55_SafeCastSpell('Zehir','RESURRECTION', allyStackId)
       _H55_SafeCastSpell('Markal','Raise_Dead',  undeadTokenOrId)  -- safely skipped

 WEEK & GROWTH:
   - Week number:
       if _H55_GetWeekNumber() >= 3 then ... end
   - Growth calculator (neutral or town math; pass your flags):
       local g = _H55_ComputeGrowth('SKELETON', true, false, true)   -- Citadel, Asha
       local g2 = _H55_ComputeGrowth(CREATURE.IMP.id, false, true, false, 1.2)

 PERKS ↔ SKILLS TABLES (queryable):
   - Skill→Perks:
       local perksOfLuck = _H55_GetPerksOfSkill('LUCK')            -- ids set (as keys)
       -- Iterate:
       local k = next(perksOfLuck, nil); while k do print('perk id',k); k = next(perksOfLuck,k) end
   - Perk→Skill:
       local base = _H55_GetSkillOfPerk('SNATCH')                  -- id of LUCK
       local ok = _H55_SkillHasPerk('LUCK', 'SNATCH')              -- true

 RESERVED / PLACEHOLDER NUMBERS (ENGINE! DO NOT REUSE):
   • CREATURE id 0   => CREATURE_UNKNOWN (placeholder/none)
   • Many engine lists treat 0 as NONE. Keep 0 unused for customs.
   • Spells/skills might reserve 0 similarly (NO_SPELL etc.).
   • TOWN/TIER/GRADE enums in this file are *framework*-side helpers, not engine IDs.

 EVERY PRESERVED FUNCTION (quick examples):
   _H55_ToUpper('dark_magic')                  --> 'DARK_MAGIC'
   _H55_Resolve('HERO_SKILL','SNATCH')         --> 149 (id)
   _H55_ConstById('HERO_SKILL',149)            --> 'HERO_SKILL_SNATCH'
   _H55_Suggest('HERO','SNAT')                 --> list like ['SNATCH', ...]
   _H55_PCall(GiveHeroSkill,'Agrael','SNATCH') --> ok, nil

   _H55_HeroHas('Agrael','SNATCH')             --> true/false
   _H55_AddHeroPerk('Agrael','SNATCH')         --> true if granted
   _H55_AddHeroSkill('Ylaya','DARK_MAGIC')     --> true (grants base)
   _H55_SetHeroSkillLevel('Ylaya','DARK_MAGIC','EXPERT')  --> true if API available
   _H55_AddHeroSkillAtLevel('Ylaya','DARK_MAGIC','EXPERT')

   _H55_CreatureHasAbility('WRAITH','UNDEAD')  --> true (if mapped in CREATURE_ABILITY_MAP)
   _H55_IsUndeadCreature('WRAITH')             --> true (if ability map says so)

   _H55_SafeCastSpell('Ylaya','LIGHTNING_BOLT', hexX, hexY)
   _H55_GetWeekNumber()                        --> 1..N
   _H55_ComputeGrowth('IMP', true, true, false)
   _H55_AddCreaturesFirstEmpty('Agrael','IMP',30)
   _H55_AddCreaturesAtSlot('Agrael', 2, 'IMP', 30)
   give_by_tier('Agrael', TOWN.HAVEN, TIER.ONE, GRADE.ONE, 40)

   _H55_GetPerksOfSkill('LUCK')
   _H55_GetSkillOfPerk('SNATCH')
   _H55_SkillHasPerk('LUCK','SNATCH')
===============================================================================
]]--[[
...
ABILITY = {
  -- NOTE: id 0 is RESERVED by engine
  ...
  LARGE_CREATURE = {
  -- NOTE: id 0 is RESERVED/invalid in most contexts; do not use. id = 67, name = 'LARGE_CREATURE', const = 'ABILITY_LARGE_CREATURE' },
...
CREATURE = {
  -- NOTE: id 0 is RESERVED/invalid in most contexts; do not use.
...
H55_ABILITY_BY_TAIL = {
  ...
  JOUSTING = { 28 },
  LARGE_CREATURE = {
  -- NOTE: id 0 is RESERVED/invalid in most contexts; do not use. 67 },
  LARGE_SHIELD = { 26 },
  LAY_HANDS = { 83 },
  LEAP = { 131 },
  LIFE_DRAIN = { 42 },
...
function _H55_ComputeGrowth(creatureToken, hasCitadel, hasCastle, hasAsha, multiplier)
  ...
  if H55_BASE == nil then
    H55_CNAME = _H55_ToUpper(creatureToken)
    H55_BASE = H55_BASE_GROWTH[H55_CNAME]
  end
  if H55_BASE == nil then H55_BASE = 0 end

  -- Multipliers: Citadel +50%, Castle +50%, Asha +50% (stacking additively),
  -- optional 'multiplier' scales the final result.
  H55_MUL = 1.0
...
TOWN = TOWN or {
  -- NOTE: 0 is RESERVED by engine (placeholder / 'NONE'). Do not use as a real town.
...
TIER = TIER or {
  -- NOTE: 0 is RESERVED by engine (placeholder / 'NONE').
  NONE  = 0,   -- RESERVED placeholder
  ONE   = 1,
  TWO   = 2,
  THREE = 3,
...
-- =====================================================================
-- v4 EXTENSIONS (Lua 4 compatible; NO 'local')  -- Generated: 2025-08-15 21:27:22 UTC
--  • Skill level helpers + safe ultimate fallback
--  • Extra queries and thorough examples for preserved functions
-- =====================================================================

-- ---------------------------------------------------------------------
-- Skill mastery levels (framework enum; not engine ids)
-- ---------------------------------------------------------------------
...
-- Resolves a level token/number into 0..3 (NONE..EXPERT).
-- Accepts: 'BASIC', 'ADV', 'ADVANCED', 'EXPERT', 1,2,3 (case-insensitive)
-- Examples:
--   _H55_ResolveSkillLevel('Expert')   --> 3
--   _H55_ResolveSkillLevel(2)          --> 2
function _H55_ResolveSkillLevel(level)
  ...
  H55_L = _H55_ToUpper(level)
  if H55_L == 'NONE' or H55_L == 'NOVICE' then return 0 end
  if H55_L == 'BASIC' then return 1 end
  if H55_L == 'ADV' or H55_L == 'ADVANCED' then return 2 end
  if H55_L == 'EXP' or H55_L == 'EXPERT' or H55_L == 'MASTER' then return 3 end
  return 0
end

-- ---------------------------------------------------------------------
-- Ultimates guard (ABSOLUTE_*); optional fallback mapping
-- ---------------------------------------------------------------------
H55_ULTIMATE_IDS = H55_ULTIMATE_IDS or {
  [85]=true,   -- HERO_SKILL_ABSOLUTE_CHARGE
  [98]=true,   -- HERO_SKILL_ABSOLUTE_GATING
  [111]=true,  -- HERO_SKILL_ABSOLUTE_FEAR
  [124]=true,  -- HERO_SKILL_ABSOLUTE_LUCK
  [137]=true,  -- HERO_SKILL_ABSOLUTE_WIZARDY
  [150]=true,  -- HERO_SKILL_ABSOLUTE_CHAINS
  [167]=true,  -- HERO_SKILL_ABSOLUTE_PROTECTION
  [176]=true,  -- HERO_SKILL_ABSOLUTE_RAGE
}

-- You may configure specific fallbacks here (ids). Empty by default.
-- Example (prefer critical gating instead of ultimate):
--   H55_ULTIMATE_FALLBACK[HERO_SKILL_ABSOLUTE_GATING] = HERO_SKILL_CRITICAL_GATING
H55_ULTIMATE_FALLBACK = H55_ULTIMATE_FALLBACK or { }

-- Returns true if token/id denotes an Ultimate.
-- Examples:
--   _H55_IsUltimateToken('ABSOLUTE_LUCK')     --> true
--   _H55_IsUltimateToken(HERO_SKILL_ABSOLUTE_GATING) --> true
function _H55_IsUltimateToken(tokenOrId)
  H55_ID = _H55_Resolve('HERO_SKILL', tokenOrId)
  if not H55_ID then return false end
  return H55_ULTIMATE_IDS[H55_ID] == true
end

-- ---------------------------------------------------------------------
-- RE-DEFINE grant helpers with Ultimate guard and messages
-- ---------------------------------------------------------------------

--[=[
_H55_AddHeroPerk(heroName, perkToken) -> boolean granted

Adds a *perk* (or base skill; both share the same HERO_SKILL id space).
Now guards against ABSOLUTE_* ultimates (refuses by default), but will
fallback to H55_ULTIMATE_FALLBACK[id] if you configured one.

Examples:
  _H55_AddHeroPerk('Agrael','SNATCH')
  _H55_AddHeroPerk('Agrael','ABSOLUTE_GATING') -- prints warning, refuses
  H55_ULTIMATE_FALLBACK[HERO_SKILL_ABSOLUTE_GATING] = HERO_SKILL_CRITICAL_GATING
  _H55_AddHeroPerk('Agrael','ABSOLUTE_GATING') -- grants CRITICAL_GATING now
]=]
function _H55_AddHeroPerk(heroName, perkToken)
  H55_ID = _H55_Resolve('HERO_SKILL', perkToken)
  if H55_ID == nil then print('_H55_AddHeroPerk: unknown token '..tostring(perkToken)); return false end
  ...
  -- Ultimate guard
  if H55_ULTIMATE_IDS[H55_ID] then
    H55_FB = H55_ULTIMATE_FALLBACK[H55_ID]
    if H55_FB then
      print('_H55_AddHeroPerk: ABSOLUTE_* blocked, using fallback id '..tostring(H55_FB))
      if HasHeroSkill(heroName, H55_FB) then return false end
      GiveHeroSkill(heroName, H55_FB)
      return true
    end
    print('_H55_AddHeroPerk: ultimates are disabled in MMH55. Refused '..tostring(perkToken))
    return false
  end
...
--[=[
_H55_AddHeroSkill(heroName, skillToken) -> boolean granted
Semantic alias of _H55_AddHeroPerk with the same ultimate guard.

Examples:
  _H55_AddHeroSkill('Ylaya','DARK_MAGIC')
  _H55_AddHeroSkill('Zehir','ABSOLUTE_WIZARDY') -- refused (or fallback if configured)
]=]
function _H55_AddHeroSkill(heroName, skillToken)
  return _H55_AddHeroPerk(heroName, skillToken)
end

-- Also refresh _H55_EnsureHeroHas to honor the ultimate guard.
-- Example:
--   _H55_EnsureHeroHas('Agrael','SNATCH')
function _H55_EnsureHeroHas(heroName, token)
  if _H55_HeroHas(heroName, token) then return false end
  return _H55_AddHeroPerk(heroName, token)
end

-- ---------------------------------------------------------------------
-- Skill level helpers (try engine API, fallback safely)
-- ---------------------------------------------------------------------

--[=[
_H55_SetHeroSkillLevel(heroName, skillToken, levelToken) -> boolean ok

Tries to set mastery level for a *base* skill (e.g., DARK_MAGIC) to
BASIC/ADVANCED/EXPERT (1..3). It attempts several engine functions in
this order (first that exists in your build is used):
  • SetHeroSkillMastery(heroName, skillId, level)
  • SetSkillMastery(heroName, skillId, level)

Fallback behavior:
  • If no known API exists, ensures the base skill is granted, prints a
    message, and returns true *only if* level <= BASIC. For higher
    levels it returns false (not supported by your build).

Examples:
  _H55_SetHeroSkillLevel('Ylaya','DARK_MAGIC','EXPERT')
  _H55_SetHeroSkillLevel('Findan','LIGHT_MAGIC', 2)
]=]
function _H55_SetHeroSkillLevel(heroName, skillToken, levelToken)
  H55_SK_ID = _H55_Resolve('HERO_SKILL', skillToken)
  if not H55_SK_ID then print('_H55_SetHeroSkillLevel: unknown skill '..tostring(skillToken)); return false end

  -- Refuse ultimate in case someone passes one here.
  if H55_ULTIMATE_IDS[H55_SK_ID] then
    print('_H55_SetHeroSkillLevel: ultimates are disabled; refusing '..tostring(skillToken))
    return false
  end

  H55_LV = _H55_ResolveSkillLevel(levelToken)
  if H55_LV <= 0 then H55_LV = SKILL_LEVEL.BASIC end

  -- Ensure base is present
  if not HasHeroSkill(heroName, H55_SK_ID) then GiveHeroSkill(heroName, H55_SK_ID) end

  -- Try known engine entry points
  if type(SetHeroSkillMastery) == 'function' then
    H55_OK = _H55_PCall(SetHeroSkillMastery, heroName, H55_SK_ID, H55_LV)
    if H55_OK then return true end
  end
  if type(SetSkillMastery) == 'function' then
    H55_OK2 = _H55_PCall(SetSkillMastery, heroName, H55_SK_ID, H55_LV)
    if H55_OK2 then return true end
  end

  -- Fallback rule
  if H55_LV <= SKILL_LEVEL.BASIC then
    print('_H55_SetHeroSkillLevel: no mastery API, ensured BASIC for '..tostring(skillToken))
    return true
  else
    print('_H55_SetHeroSkillLevel: your build lacks mastery API; cannot set to level '..tostring(H55_LV))
    return false
  end
end

--[=[
_H55_AddHeroSkillAtLevel(heroName, skillToken, levelToken) -> boolean ok

Ensures the skill exists *and* attempts to set its mastery to the requested
level. This is the one-liner you want most of the time.

Examples:
  _H55_AddHeroSkillAtLevel('Ylaya','DARK_MAGIC','EXPERT')
  _H55_AddHeroSkillAtLevel('Findan','LIGHT_MAGIC','ADVANCED')
]=]
function _H55_AddHeroSkillAtLevel(heroName, skillToken, levelToken)
  _H55_AddHeroSkill(heroName, skillToken)
  return _H55_SetHeroSkillLevel(heroName, skillToken, levelToken)
end

-- ---------------------------------------------------------------------
-- Perk/Skill table queries
-- ---------------------------------------------------------------------

--[=[
_H55_GetPerksOfSkill(skillToken [, returnConstNames=false]) -> table-as-set or name-list

Returns the known perks of a base skill as a set of ids (keys) when
returnConstNames==false (default), or as a numerically indexed list of
const strings when true.

Examples:
  local set = _H55_GetPerksOfSkill('LUCK')
  local list = _H55_GetPerksOfSkill('LUCK', true)
]=]
function _H55_GetPerksOfSkill(skillToken, returnConstNames)
  H55_ID = _H55_Resolve('HERO_SKILL', skillToken)
  if not H55_ID then return {} end
  H55_SET = H55_PERKS_BY_SKILL and H55_PERKS_BY_SKILL[H55_ID]
  if not H55_SET then return {} end
  if not returnConstNames then return H55_SET end
  -- build list of const names
  H55_OUT = {}
  H55_K = next(H55_SET, nil)
  while H55_K do
    H55_NAME = _H55_ConstById('HERO_SKILL', H55_K) or tostring(H55_K)
    H55_OUT[_H55_Getn(H55_OUT)+1] = H55_NAME
    H55_K = next(H55_SET, H55_K)
  end
  return H55_OUT
end

--[=[
_H55_GetSkillOfPerk(perkToken [, returnConstName=false]) -> id or const

Finds the *base skill* to which the given perk belongs.

Examples:
  _H55_GetSkillOfPerk('SNATCH')         --> id of LUCK
  _H55_GetSkillOfPerk('SNATCH', true)   --> 'HERO_SKILL_LUCK'
]=]
function _H55_GetSkillOfPerk(perkToken, returnConstName)
  H55_PID = _H55_Resolve('HERO_SKILL', perkToken)
  if not H55_PID then return nil end
  H55_SID = H55_SKILL_OF_PERK and H55_SKILL_OF_PERK[H55_PID]
  if not H55_SID then return nil end
  if returnConstName then return _H55_ConstById('HERO_SKILL', H55_SID) end
  return H55_SID
end

--[=[
_H55_SkillHasPerk(skillToken, perkToken) -> boolean

Returns true if the perk belongs to the skill according to the current
skill-tree table embedded in the framework.

Examples:
  _H55_SkillHasPerk('LUCK','SNATCH')        --> true
  _H55_SkillHasPerk('LEADERSHIP','SNATCH')  --> false
]=]
function _H55_SkillHasPerk(skillToken, perkToken)
  H55_SID = _H55_Resolve('HERO_SKILL', skillToken)
  H55_PID = _H55_Resolve('HERO_SKILL', perkToken)
  if not H55_SID or not H55_PID then return false end
  H55_SET = H55_PERKS_BY_SKILL and H55_PERKS_BY_SKILL[H55_SID]
  if not H55_SET then return false end
  return H55_SET[H55_PID] == true
end

-- Thin alias for readability
-- Example: if _H55_HeroHasPerk('Agrael','SNATCH') then ... end
function _H55_HeroHasPerk(heroName, perkToken)
  return _H55_HeroHas(heroName, perkToken)
end

-- (Optional) expose the raw skill-perk table in a compact row format:
-- [['SKILL_CONST', 'PERK_CONST', 'PERK_CONST', ...], ...]
-- Useful for debugging or exporting.
function _H55_GetSkillPerkRows()
  return H55_HERO_SKILL_TREE_ROWS
end

-- ---------------------------------------------------------------------
-- Example-only adjustments for SafeCast (documentation kept near code):
--  (The implementation already prevents RAISE/RESURRECT on undead.)
--  You can extend CREATURE_ABILITY_MAP to make undead detection precise.
-- ---------------------------------------------------------------------

-- ---------------------------------------------------------------------
-- End of v4 extensions
-- ---------------------------------------------------------------------
...
--[[
===============================================================================
 MMH55 ENUM/RESOLVER FRAMEWORK — v5 (Lua 4 / Lua 3.8 compatible, NO 'local')
  *This section is an additive/override layer on top of v4 in this file.*

 WHY v5 EXISTS (what you can do now):
   • **Dot-notation arguments** everywhere:
       _H55_SetHeroSkillLevel('Ylaya', SKILL.DARK_MAGIC, LEVEL.EXPERT)
       _H55_GiveCreaturesTo('Agrael', TOWN.HAVEN, TIER.ONE, GRADE.ONE, 40, true)
       _H55_MultiplyCreaturesOfHero('Agrael', TOWN.INFERNO, TIER.THREE, GRADE.UPGRADE_A, 2, true)
       _H55_GivePerkSoft('Agrael', PERK.SNATCH, true)
       _H55_GivePerkHard('Agrael', PERK.MENTORING, LEVEL.BASIC, true)
       _H55_RemovePerk('Agrael', PERK.SNATCH, true)
       _H55_RemoveCreature('Agrael', TOWN.HAVEN, TIER.FOUR, GRADE.UPGRADE_A, 10, true)
       _H55_RemoveCreatureTown('TownOfAshwood', TOWN.NECROPOLIS, TIER.TWO, GRADE.UPGRADE_A, 12, true)
   • **All helpers can optionally show a flying text** near the hero/object.
   • **Ultimate** (“ABSOLUTE_*”) protection and fallback is still applied automatically.
   • **Shorter / reused code paths** to keep the framework maintainable.

 IMPORTANT: This v5 block *overrides* some core helpers (e.g. _H55_Resolve) to
 add dot-notation support without touching the original v4 data tables.
===============================================================================
]]

-- ---------------------------------------------------------------------------
-- Aliases to enable dot-notation exactly as requested
-- ---------------------------------------------------------------------------

-- Skill & perk enums (kept SKILL = HERO_SKILL for base skills)
if SKILL == nil then SKILL = HERO_SKILL end

-- ---------------------------------------------------------------------------
-- v6: Build a truly flat PERK table from HERO_PERK families (dot-notation safe)
-- ---------------------------------------------------------------------------
if HERO_PERK and type(PERK) ~= 'table' then
  PERK = {}
  H55__PF_FAM = next(HERO_PERK, nil)
  while H55__PF_FAM do
    H55__PF_OBJ = HERO_PERK[H55__PF_FAM]
    if H55__PF_OBJ and type(H55__PF_OBJ.items) == 'table' then
      H55__PF_ITEM = next(H55__PF_OBJ.items, nil)
      while H55__PF_ITEM do
        H55__PF_ENTRY = H55__PF_OBJ.items[H55__PF_ITEM]
        if H55__PF_ENTRY and H55__PF_ENTRY.id then
          PERK[H55__PF_ENTRY.name or H55__PF_ITEM] = {
            id = H55__PF_ENTRY.id,
            name = H55__PF_ENTRY.name or H55__PF_ITEM,
            const = H55__PF_ENTRY.const
          }
        end
        H55__PF_ITEM = next(H55__PF_OBJ.items, H55__PF_ITEM)
      end
    end
    H55__PF_FAM = next(HERO_PERK, H55__PF_FAM)
  end
end

-- Mastery levels alias (BASIC/ADVANCED/EXPERT) as LEVEL
if LEVEL == nil and SKILL_LEVEL ~= nil then LEVEL = SKILL_LEVEL end

-- TOWN.HEAVEN typo alias for HAVEN
if type(TOWN) == 'table' and TOWN.HEAVEN == nil and TOWN.HAVEN ~= nil then
  TOWN.HEAVEN = TOWN.HAVEN
end


-- TOWN.HEAVEN is a common typo; alias it to HAVEN for convenience
if type(TOWN) == 'table' and TOWN.HEAVEN == nil and TOWN.HAVEN ~= nil then
  TOWN.HEAVEN = TOWN.HAVEN
end

-- ---------------------------------------------------------------------------
-- Robust token/id resolver that accepts dot-notation tables transparently
-- ---------------------------------------------------------------------------

if __H55_Resolve_OLD == nil then __H55_Resolve_OLD = _H55_Resolve end

--[[
_H55_Resolve(domain, tokenOrIdOrTable) -> numeric id

v5 extension:
  - Accepts objects like SKILL.DARK_MAGIC, LEVEL.EXPERT, CREATURE.IMP, etc.
  - Falls back to the original v4 resolver for strings and numbers.
]]
function _H55_Resolve(domain, token)
  ...
  -- Case 2: dot-notation object { id=..., name=..., const=... }
  if type(token) == 'table' then
    if token.id then return token.id end
    if token.const then return __H55_Resolve_OLD(domain, token.const) end
    if token.name then return __H55_Resolve_OLD(domain, token.name) end
    -- some enums store single value at [1]
    if token[1] then return __H55_Resolve_OLD(domain, token[1]) end
    return nil
  end

  -- Case 3: string token/const -> use the original v4 resolver
  return __H55_Resolve_OLD(domain, token)
end

-- ---------------------------------------------------------------------------
-- Flying text (debug/feedback). Never crashes if UI helpers are missing.
-- ---------------------------------------------------------------------------
-- Shows a flying sign near an object (hero/town) if supported; otherwise
-- prints to log. Returns true if *attempted* to show.
-- Examples:
--   _H55_Fly('Ylaya', 'PERK GIVEN: SNATCH')
--   _H55_Fly('Agrael', 'CREATURES GIVEN: Haven T1.G1 x40')
function _H55_Fly(objectName, text)
  if not text or text == '' then return false end
  -- 3 safe attempts with graceful fallback; engine variants differ between maps
  if type(ShowFlyingSign) == 'function' then
    _H55_SafeCallByName('ShowFlyingSign', text, objectName)
    return true
  end
  if type(StartThread) == 'function' and type(FlyingSign) == 'function' then
    _H55_SafeCallByName('StartThread', FlyingSign, text, objectName)
    return true
  end
  print('[FLY]['..tostring(objectName)..'] '..tostring(text))
  return true
end

-- Compose a short readable tag for town/tier/grade (for messages)
function _H55_TTG_ToString(town, tier, grade)
  return tostring(town) .. ' T' .. tostring(tier) .. '.G' .. tostring(grade)
end

-- Resolve a TOWNxTIERxGRADE into a creature id using v4's FACTION_UNITS table.
-- Returns creatureId or nil.
function _H55_ResolveCreatureByTTG(town, tier, grade)
  if FACTION_UNITS == nil then return nil end
  localTown = town
  -- v4 allowed mixed types; support dot-notation tables as well
  if type(town) == 'table' and town.id then localTown = town.id end
  if type(town) == 'table' and town[1] then localTown = town[1] end

  localTier  = tier
  if type(tier) == 'table' and tier.id then localTier = tier.id end
  if type(tier) == 'table' and tier[1] then localTier = tier[1] end

  localGrade = grade
  if type(grade) == 'table' and grade.id then localGrade = grade.id end
  if type(grade) == 'table' and grade[1] then localGrade = grade[1] end

  H55_FAC = FACTION_UNITS[localTown]
  if H55_FAC == nil then return nil end
  H55_T = H55_FAC[localTier]
  if H55_T == nil then return nil end
  H55_G = H55_T[localGrade]
  if H55_G == nil then return nil end
  return H55_G
end

-- ---------------------------------------------------------------------------
-- Generic army slot helpers (work for heroes, towns, garrisons)
-- ---------------------------------------------------------------------------

-- Iterates slots 0..6; calls cb(objectName, slot, creatureId, quantity). If cb
-- returns a truthy value, iteration stops early.
function _H55_ForEachArmySlot(objectName, cb)
  H55_SLOT = 0
  while H55_SLOT <= 6 do
    H55_ID, H55_QTY = nil, 0
    if type(GetObjectArmySlotCreature) == 'function' then
      H55_ID = GetObjectArmySlotCreature(objectName, H55_SLOT)
    end
    if type(GetObjectArmySlotQuantity) == 'function' then
      H55_QTY = GetObjectArmySlotQuantity(objectName, H55_SLOT) or 0
    end
    if cb(objectName, H55_SLOT, H55_ID, H55_QTY) then return true end
    H55_SLOT = H55_SLOT + 1
  end
  return false
end

-- Returns slot index that contains creatureId, or -1 if not found.
function _H55_FindSlotWith(objectName, creatureId)
  H55_FOUND = -1
  _H55_ForEachArmySlot(objectName, function(obj, slot, cid, qty)
    if cid == creatureId and qty > 0 then H55_FOUND = slot; return true end
    return false
  end)
  return H55_FOUND
end

-- Ensures a creature is added to the first empty slot (uses v4 function if present).
function _H55_AddFirstEmpty(objectName, creatureId, count)
  if type(_H55_AddCreaturesFirstEmpty) == 'function' then
    return _H55_AddCreaturesFirstEmpty(objectName, creatureId, count)
  end
  -- fallback: scan slots manually
  return _H55_ForEachArmySlot(objectName, function(obj, slot, cid, qty)
    if cid == nil or cid == 0 or qty == 0 then
      if type(SetObjectArmySlotCreature) == 'function' then
        SetObjectArmySlotCreature(obj, slot, creatureId)
      end
      if type(SetObjectArmySlotQuantity) == 'function' then
        SetObjectArmySlotQuantity(obj, slot, count)
      else
        AddHeroCreatures(obj, creatureId, count)
      end
      return true
    end
  end)
...
-- ---------------------------------------------------------------------------
-- DOT-NOTATION friendly HIGH-LEVEL HELPERS (with optional flying text)
-- ---------------------------------------------------------------------------

--[[
_H55_GiveCreaturesTo(heroName, town, tier, grade, amount, showText=false) -> boolean

Adds `amount` of the resolved creature to the first empty slot of `heroName`.

Examples:
  _H55_GiveCreaturesTo('Agrael', TOWN.HAVEN, TIER.ONE, GRADE.ONE, 40, true)
  _H55_GiveCreaturesTo('Agrael', TOWN.INFERNO, TIER.THREE, GRADE.UPGRADE_A, 12)
]]
function _H55_GiveCreaturesTo(heroName, town, tier, grade, amount, showText)
  H55_CID = _H55_ResolveCreatureByTTG(town, tier, grade)
  if H55_CID == nil then
    print('_H55_GiveCreaturesTo: cannot resolve creature for '.._H55_TTG_ToString(town,tier,grade))
    return false
  end
  _H55_AddFirstEmpty(heroName, H55_CID, amount)
  if showText then _H55_Fly(heroName, 'CREATURES GIVEN '.._H55_TTG_ToString(town,tier,grade)..' x'..tostring(amount)) end
  return true
end

--[[
_H55_MultiplyCreaturesOfHero(heroName, town, tier, grade, factor, showText=false) -> boolean

If the hero has that stack, multiply its quantity by `factor` (floored).

Examples:
  _H55_MultiplyCreaturesOfHero('Agrael', TOWN.INFERNO, TIER.THREE, GRADE.UPGRADE_A, 2, true)
]]
function _H55_MultiplyCreaturesOfHero(heroName, town, tier, grade, factor, showText)
  H55_CID = _H55_ResolveCreatureByTTG(town, tier, grade)
  if H55_CID == nil then
    print('_H55_MultiplyCreaturesOfHero: cannot resolve creature for '.._H55_TTG_ToString(town,tier,grade))
    return false
  end
  H55_SLOT = _H55_FindSlotWith(heroName, H55_CID)
  if H55_SLOT < 0 then return false end
  if type(GetObjectArmySlotQuantity) ~= 'function' then
    print('_H55_MultiplyCreaturesOfHero: slot quantity API not available')
    return false
  end
  H55_Q = GetObjectArmySlotQuantity(heroName, H55_SLOT) or 0
  H55_NEW = math.floor(H55_Q * (factor or 1))
  if H55_NEW < 0 then H55_NEW = 0 end
  if type(SetObjectArmySlotQuantity) == 'function' then
    SetObjectArmySlotQuantity(heroName, H55_SLOT, H55_NEW)
  else
    -- best-effort fallback: add the delta (may be slightly off if caps apply)
    if H55_NEW > H55_Q then
      AddHeroCreatures(heroName, H55_CID, H55_NEW - H55_Q)
    elseif H55_NEW < H55_Q and type(RemoveHeroCreatures) == 'function' then
      RemoveHeroCreatures(heroName, H55_CID, H55_Q - H55_NEW)
    end
  end
  if showText then _H55_Fly(heroName, 'CREATURES MULTIPLIED '.._H55_TTG_ToString(town,tier,grade)..' x'..tostring(factor)) end
  return true
end

--[[
_H55_RemoveCreature(objectName, town, tier, grade, amount, showText=false) -> boolean

Removes up to `amount` of the target creature from the first matching stack of
the given hero (or any object with an army).

Example:
  _H55_RemoveCreature('Agrael', TOWN.HAVEN, TIER.FOUR, GRADE.UPGRADE_A, 10, true)
]]
function _H55_RemoveCreature(objectName, town, tier, grade, amount, showText)
  H55_CID = _H55_ResolveCreatureByTTG(town, tier, grade)
  if H55_CID == nil then
    print('_H55_RemoveCreature: cannot resolve creature for '.._H55_TTG_ToString(town,tier,grade))
    return false
  end
  H55_SLOT = _H55_FindSlotWith(objectName, H55_CID)
  if H55_SLOT < 0 then return false end

  H55_Q = (type(GetObjectArmySlotQuantity) == 'function') and (GetObjectArmySlotQuantity(objectName, H55_SLOT) or 0) or 0
  H55_DEL = amount or H55_Q
  if H55_DEL >= H55_Q then
    if type(SetObjectArmySlotQuantity) == 'function' then
      SetObjectArmySlotQuantity(objectName, H55_SLOT, 0)
    elseif type(RemoveHeroCreatures) == 'function' then
      RemoveHeroCreatures(objectName, H55_CID, H55_Q)
    end
  else
    if type(SetObjectArmySlotQuantity) == 'function' then
      SetObjectArmySlotQuantity(objectName, H55_SLOT, H55_Q - H55_DEL)
    elseif type(RemoveHeroCreatures) == 'function' then
      RemoveHeroCreatures(objectName, H55_CID, H55_DEL)
    end
  end
  if showText then _H55_Fly(objectName, 'CREATURES REMOVED '.._H55_TTG_ToString(town,tier,grade)..' x'..tostring(amount)) end
  return true
end

--[[
_H55_RemoveCreatureTown(townObjectName, town, tier, grade, amount, showText=false)

Same as _H55_RemoveCreature but intended for a town/garrison object.

Example:
  _H55_RemoveCreatureTown('TownOfAshwood', TOWN.NECROPOLIS, TIER.TWO, GRADE.UPGRADE_A, 12, true)
]]
function _H55_RemoveCreatureTown(townObjectName, town, tier, grade, amount, showText)
  return _H55_RemoveCreature(townObjectName, town, tier, grade, amount, showText)
end

-- ---------------------------------------------------------------------------
-- Skill mastery helpers with DOT notation and H55 fallback
-- ---------------------------------------------------------------------------

-- Ensures level is one of BASIC/ADVANCED/EXPERT; ultimates are not allowed.
function _H55__ClampLevel(levelToken)
  H55_L = _H55_Resolve('SKILL_LEVEL', levelToken)
  if H55_L == nil and LEVEL ~= nil then H55_L = _H55_Resolve('SKILL_LEVEL', LEVEL.BASIC) end
  if H55_L == nil then H55_L = 1 end -- BASIC as ultimate fallback
  if H55_L > 3 then H55_L = 3 end    -- cap at EXPERT (H55 has no ultimates)
  if H55_L < 1 then H55_L = 1 end
  return H55_L
end

--[[
_h55_UpgradeSkill(heroName, skill, level, showText=false) -> boolean
Upgrades an existing skill to the given mastery (no-op if missing).

Examples:
  _h55_UpgradeSkill('Ylaya', SKILL.DARK_MAGIC, LEVEL.EXPERT, true)
]]
function _h55_UpgradeSkill(heroName, skillToken, levelToken, showText)
  H55_SK = _H55_Resolve('HERO_SKILL', skillToken)
  if H55_SK == nil then return false end
  if not _H55_HeroHas(heroName, H55_SK) then return false end
  H55_LV = _H55__ClampLevel(levelToken)
  if _H55_SetHeroSkillLevel then _H55_SetHeroSkillLevel(heroName, H55_SK, H55_LV) end
  if showText then _H55_Fly(heroName, 'SKILL UPGRADED '..tostring(H55_SK)..' -> L'..tostring(H55_LV)) end
  return true
end

--[[
_h55_GiveSkill(heroName, skill, level=BASIC, showText=false) -> boolean
Gives a skill at level (if free slot) otherwise shows a message.

Examples:
  _h55_GiveSkill('Ylaya', SKILL.DARK_MAGIC, LEVEL.ADVANCED, true)
  _h55_GiveSkill('Ylaya', SKILL.DARK_MAGIC, nil, true) -- BASIC by default
]]
function _h55_GiveSkill(heroName, skillToken, levelToken, showText)
  H55_SK = _H55_Resolve('HERO_SKILL', skillToken)
  if H55_SK == nil then return false end
  if _H55_HeroHas(heroName, H55_SK) then
    -- already has; consider upgrade to requested level
    if levelToken then _h55_UpgradeSkill(heroName, H55_SK, levelToken, showText) end
    return true
  end
  -- slots check (best-effort; engine exposes occupied count & limit variably)
  H55_SLOTS_OK = true
  if type(GetHeroSkillsCount) == 'function' and type(GetHeroMaxSkills) == 'function' then
    H55_CUR = GetHeroSkillsCount(heroName)
    H55_MAX = GetHeroMaxSkills(heroName)
    if H55_CUR ~= nil and H55_MAX ~= nil and H55_CUR >= H55_MAX then
      H55_SLOTS_OK = false
    end
  end
  if not H55_SLOTS_OK then
    if showText then _H55_Fly(heroName, 'NO FREE SKILL SLOT') end
    return false
  end
  _H55_AddHeroSkill(heroName, H55_SK)
  H55_LV = _H55__ClampLevel(levelToken or LEVEL and LEVEL.BASIC or 1)
  if _H55_SetHeroSkillLevel then _H55_SetHeroSkillLevel(heroName, H55_SK, H55_LV) end
  if showText then _H55_Fly(heroName, 'SKILL GIVEN id='..tostring(H55_SK)..' L'..tostring(H55_LV)) end
  return true
end

-- Soft/hard perk grants with automatic base-skill resolution
-- (uses v4: _H55_GetSkillOfPerk & _H55_AddHeroPerk)

--[[
_h55_GivePerkSoft(heroName, perk, showText=false) -> boolean
Gives the perk only if its base skill already exists on the hero.
Example:
  _h55_GivePerkSoft('Agrael', PERK.SNATCH, true)
]]
function _h55_GivePerkSoft(heroName, perkToken, showText)
  H55_PK = _H55_Resolve('HERO_SKILL', perkToken)
  if H55_PK == nil then return false end
  H55_SK = _H55_GetSkillOfPerk and _H55_GetSkillOfPerk(H55_PK) or nil
  if H55_SK and not _H55_HeroHas(heroName, H55_SK) then
    if showText then _H55_Fly(heroName, 'PERK SKIPPED (no base skill) id='..tostring(H55_PK)) end
    return false
  end
  if _H55_AddHeroPerk(heroName, H55_PK) then
    if showText then _H55_Fly(heroName, 'PERK GIVEN id='..tostring(H55_PK)) end
    return true
  end
  return false
end
...
--[[
_h55_GivePerkHard(heroName, perk, levelForBase=BASIC, showText=false) -> boolean
If the base skill is absent, it is granted at the given mastery first.
Example:
  _h55_GivePerkHard('Agrael', PERK.SNATCH, LEVEL.BASIC, true)
]]
function _h55_GivePerkHard(heroName, perkToken, levelForBase, showText)
  H55_PK = _H55_Resolve('HERO_SKILL', perkToken)
  if H55_PK == nil then return false end
  H55_SK = _H55_GetSkillOfPerk and _H55_GetSkillOfPerk(H55_PK) or nil
  if H55_SK and not _H55_HeroHas(heroName, H55_SK) then
    _h55_GiveSkill(heroName, H55_SK, levelForBase or LEVEL and LEVEL.BASIC or 1, showText)
  end
  if _H55_AddHeroPerk(heroName, H55_PK) then
    if showText then _H55_Fly(heroName, 'PERK GIVEN id='..tostring(H55_PK)) end
    return true
...
--[[
_h55_RemovePerk(heroName, perk, showText=false) -> boolean
Removes the perk if present.
Example:
  _h55_RemovePerk('Agrael', PERK.SNATCH, true)
]]
function _h55_RemovePerk(heroName, perkToken, showText)
  H55_PK = _H55_Resolve('HERO_SKILL', perkToken)
  if H55_PK == nil then return false end
  if _H55_HeroHas(heroName, H55_PK) and type(RemoveHeroSkill) == 'function' then
    RemoveHeroSkill(heroName, H55_PK)
    if showText then _H55_Fly(heroName, 'PERK REMOVED id='..tostring(H55_PK)) end
    return true
  end
  return false
end
...
-- ---------------------------------------------------------------------------
-- Friendly name aliases in lowercase (requested examples used _h55_*)
-- ---------------------------------------------------------------------------
h55_GiveCreaturesTo        = _H55_GiveCreaturesTo
h55_MultiplyCreaturesOfHero= _H55_MultiplyCreaturesOfHero
h55_RemoveCreature         = _H55_RemoveCreature
h55_RemoveCreatureTown     = _H55_RemoveCreatureTown
h55_UpgradeSkill           = _h55_UpgradeSkill
h55_GiveSkill              = _h55_GiveSkill
h55_GivePerkSoft           = _h55_GivePerkSoft
h55_GivePerkHard           = _h55_GivePerkHard
h55_RemovePerk             = _h55_RemovePerk


-- =====================================================================
-- v6: Uppercase wrappers for lowercase helpers (API parity & consistency)
-- =====================================================================
if type(_h55_GivePerkSoft) == 'function' and type(_H55_GivePerkSoft) ~= 'function' then
  function _H55_GivePerkSoft(heroName, perkToken, showText)
    return _h55_GivePerkSoft(heroName, perkToken, showText)
  end
end

if type(_h55_GivePerkHard) == 'function' and type(_H55_GivePerkHard) ~= 'function' then
  function _H55_GivePerkHard(heroName, perkToken, levelForBase, showText)
    return _h55_GivePerkHard(heroName, perkToken, levelForBase, showText)
  end
end

if type(_h55_RemovePerk) == 'function' and type(_H55_RemovePerk) ~= 'function' then
  function _H55_RemovePerk(heroName, perkToken, showText)
    return _h55_RemovePerk(heroName, perkToken, showText)
  end
end

if type(_h55_GiveSkill) == 'function' and type(_H55_GiveSkill) ~= 'function' then
  function _H55_GiveSkill(heroName, skillToken, levelToken, showText)
    return _h55_GiveSkill(heroName, skillToken, levelToken, showText)
  end
end

if type(_h55_UpgradeSkill) == 'function' and type(_H55_UpgradeSkill) ~= 'function' then
  function _H55_UpgradeSkill(heroName, skillToken, levelToken, showText)
    return _h55_UpgradeSkill(heroName, skillToken, levelToken, showText)
  end
end
...
--[[
================================================================================
  MMH55 ENUM/RESOLVER FRAMEWORK — v7 (Lua 4 / Lua 3.8 compatible, NO 'local')
  Generated: 2025-08-16 09:01:01 UTC
  Changes from v6 (requested by user):
    • Comprehensive **spell ENUMs by school** (dot notation) with lazy resolution.
      NOTE: IDs use base Heroes V constants (DEFAULT USED where H55-specific is unknown).
    • New immunity catalogue & rules used by the new _H55_CastSpell().
      Covers BLIND, SLOW, WEAKNESS, CONFUSION, PUPPET_MASTER, FRENZY and generic
      MAGIC immunity, mind-control protection, elemental immunities (DEFAULT USED).
      Also treats a unit under Vampirism as Undead (if engine exposes this status).
    • Growth system finalized: adds **tier defaults** and a safer, faster
      _H55_ComputeGrowth() override that falls back to per-tier growth if a
      per-creature entry is missing. Castle/Citadel/Asha stacking kept.
    • pcall helpers: added _H55_PCallT (table passthrough) alongside existing
      _H55_PCall (expanded returns); internal calls use the expanded form but
      devs can now opt into the lighter table form.
    • Better UX: every public API can emit flying text (optional bool) with clear,
      consistent messages like "PERK GIVEN {...}" and "CREATURES GIVEN {...}".
    • Code cleanup: removed redundant branches, added fast caches, normalized
      guard clauses, and ensured no 'local' usage per Lua 4 constraint.

  -----------------------------------------------------------------------------
  QUICK USE CASES (new / updated):
    -- Skills / mastery (ultimates auto-fallback to EXPERT in MMH55):
    _H55_GiveSkill('Ylaya', SKILL.DARK_MAGIC, LEVEL.BASIC, true)
    _H55_UpgradeSkill('Ylaya', SKILL.DARK_MAGIC, LEVEL.EXPERT, true)
    _H55_SetHeroSkillLevel('Ylaya', SKILL.DARK_MAGIC, LEVEL.EXPERT) -- dot notation

    -- Perks:
    _H55_GivePerkSoft('Agrael', PERK.SNATCH, true)                     -- requires skill
    _H55_GivePerkHard('Vittorio', PERK.MENTORING, LEVEL.BASIC, true)    -- auto-grants skill
    _H55_RemovePerk('Agrael', PERK.SNATCH, true)

    -- Creatures by town/tier/grade (dot notation):
    _H55_GiveCreaturesTo('Ylaya',   TOWN.DUNGEON, TIER.FIVE, GRADE.ONE,  40, true)
    _H55_MultiplyCreaturesOfHero('Ylaya', TOWN.DUNGEON, TIER.FIVE, GRADE.ONE, 2, true)
    _H55_RemoveCreature('Ylaya',    TOWN.DUNGEON, TIER.FIVE, GRADE.ONE,  10, true)

    -- Safer spell-casting with power parameter & immunity checks:
    _H55_CastSpell('Ylaya', SPELL.DARK.BLIND, enemyUnitId, nil, nil, true) -- DEFAULT USED
    _H55_CastSpell('Zehir', SPELL.LIGHT.RESURRECTION, allyUnitId, nil, 12, true)
    _H55_SafeCastSpell('Zehir', SPELL.LIGHT.RESURRECTION, allyUnitId)       -- legacy alias

    -- Growth (town/neutral; per-creature overrides + per-tier defaults):
    -- DEFAULT USED growth tiers; replace with MMH55 exact if needed.
    _H55_ComputeGrowth('PEASANT', true, true, false)              -- by creature token
    _H55_ComputeGrowthByTier(TIER.THREE, true, false, false, 1.2) -- by tier only

  IMPORTANT: All spell lists and immunity rules that are not MMH55-specific are
  marked **DEFAULT USED** (from Heroes V base logic). Replace/update freely to
  match MMH55 balance once a definitive list is supplied.
================================================================================
]]

-- ============================================================================
--  SPELL ENUMS (schools -> spells)  **DEFAULT USED**
--  We intentionally store *const strings* (e.g., 'SPELL_RESURRECTION') and
--  resolve them to numeric ids lazily via _H55_Resolve('SPELL', token).
--  This avoids hardcoding ids and remains forward compatible with MMH55.
-- ============================================================================
if not SPELL then SPELL = {} end
if not SPELL.LIGHT then SPELL.LIGHT = {} end
if not SPELL.DARK then SPELL.DARK = {} end
if not SPELL.DESTRUCTIVE then SPELL.DESTRUCTIVE = {} end
if not SPELL.SUMMONING then SPELL.SUMMONING = {} end
if not SPELL.SPECIAL then SPELL.SPECIAL = {} end
if not SPELL.BATTLE_CRY then SPELL.BATTLE_CRY = {} end  -- aka Warcries

-- LIGHT magic (DEFAULT USED from H5)
SPELL.LIGHT = SPELL.LIGHT or {}
SPELL.LIGHT.DIVINE_STRENGTH   = 'SPELL_DIVINE_STRENGTH'
SPELL.LIGHT.RIGHTEOUS_MIGHT   = 'SPELL_RIGHTEOUS_MIGHT'
SPELL.LIGHT.HASTE             = 'SPELL_HASTE'
SPELL.LIGHT.ENDURANCE         = 'SPELL_ENDURANCE'
SPELL.LIGHT.CLEANSING         = 'SPELL_CLEANSING'
SPELL.LIGHT.DEFLECT_MISSILE   = 'SPELL_DEFLECT_MISSILE'       -- sometimes DEFLECT_ARROWS
SPELL.LIGHT.WORD_OF_LIGHT     = 'SPELL_WORD_OF_LIGHT'
SPELL.LIGHT.RESURRECTION      = 'SPELL_RESURRECTION'
SPELL.LIGHT.DIVINE_VENGEANCE  = 'SPELL_DIVINE_VENGEANCE'

-- DARK magic (DEFAULT USED from H5)
SPELL.DARK = SPELL.DARK or {}
SPELL.DARK.SLOW               = 'SPELL_SLOW'
SPELL.DARK.WEAKNESS           = 'SPELL_WEAKNESS'
SPELL.DARK.CONFUSION          = 'SPELL_CONFUSION'
SPELL.DARK.SUFFERING          = 'SPELL_SUFFERING'
SPELL.DARK.VULNERABILITY      = 'SPELL_VULNERABILITY'
SPELL.DARK.DECAY              = 'SPELL_DECAY'
SPELL.DARK.BLIND              = 'SPELL_BLIND'
SPELL.DARK.PUPPET_MASTER      = 'SPELL_PUPPET_MASTER'
SPELL.DARK.FRENZY             = 'SPELL_FRENZY'
SPELL.DARK.VAMPIRISM          = 'SPELL_VAMPIRISM'             -- makes living unit act Undead (DEFAULT USED)

-- DESTRUCTIVE magic (DEFAULT USED from H5)
SPELL.DESTRUCTIVE = SPELL.DESTRUCTIVE or {}
SPELL.DESTRUCTIVE.MAGIC_ARROW   = 'SPELL_MAGIC_ARROW'
SPELL.DESTRUCTIVE.LIGHTNING_BOLT= 'SPELL_LIGHTNING_BOLT'
SPELL.DESTRUCTIVE.CHAIN_LIGHTNING= 'SPELL_CHAIN_LIGHTNING'
SPELL.DESTRUCTIVE.ICE_BOLT      = 'SPELL_ICE_BOLT'            -- aka Ice Bolt
SPELL.DESTRUCTIVE.CIRCLE_OF_WINTER= 'SPELL_CIRCLE_OF_WINTER'
SPELL.DESTRUCTIVE.DEEP_FREEZE   = 'SPELL_DEEP_FREEZE'
SPELL.DESTRUCTIVE.FIREBALL      = 'SPELL_FIREBALL'
SPELL.DESTRUCTIVE.FIREWALL      = 'SPELL_FIREWALL'
SPELL.DESTRUCTIVE.METEOR_SHOWER = 'SPELL_METEOR_SHOWER'
SPELL.DESTRUCTIVE.ARMAGEDDON    = 'SPELL_ARMAGEDDON'

-- SUMMONING magic (DEFAULT USED from H5)
SPELL.SUMMONING = SPELL.SUMMONING or {}
SPELL.SUMMONING.MAGIC_FIST      = 'SPELL_MAGIC_FIST'          -- Fist of Wrath
SPELL.SUMMONING.PHANTOM         = 'SPELL_PHANTOM'             -- Phantom Forces
SPELL.SUMMONING.RAISE_DEAD      = 'SPELL_RAISE_DEAD'
SPELL.SUMMONING.WASP_SWARM      = 'SPELL_WASP_SWARM'
SPELL.SUMMONING.EARTHQUAKE      = 'SPELL_EARTHQUAKE'
SPELL.SUMMONING.BLADE_BARRIER   = 'SPELL_BLADE_BARRIER'
SPELL.SUMMONING.ARCANE_CRYSTAL  = 'SPELL_ARCANE_CRYSTAL'
SPELL.SUMMONING.SUMMON_ELEMENTALS= 'SPELL_SUMMON_ELEMENTALS'
SPELL.SUMMONING.SUMMON_PHOENIX  = 'SPELL_SUMMON_PHOENIX'
SPELL.SUMMONING.TELEPORTATION   = 'SPELL_TELEPORTATION'

-- SPECIAL / engine effects (DEFAULT USED: naming varies between maps)
SPELL.SPECIAL = SPELL.SPECIAL or {}
SPELL.SPECIAL.RETALIATION      = 'SPELL_RETALIATION'          -- placeholder/engine
SPELL.SPECIAL.REGENERATION     = 'SPELL_REGENERATION'         -- engine effect (creature ability too)

-- Warcries (aka BATTLE_CRY) — names differ across mods; provide common aliases.
SPELL.BATTLE_CRY = SPELL.BATTLE_CRY or {}
SPELL.BATTLE_CRY.CALL_OF_BLOOD  = 'SPELL_CALL_OF_BLOOD'       -- DEFAULT USED
SPELL.BATTLE_CRY.ANGER_CALL     = 'SPELL_ANGER_CALL'          -- requested alias (DEFAULT USED)
SPELL.BATTLE_CRY.FEAR_MY_ROAR   = 'SPELL_FEAR_MY_ROAR'        -- DEFAULT USED
SPELL.BATTLE_CRY.VOICE_OF_RAGE  = 'SPELL_VOICE_OF_RAGE'       -- DEFAULT USED

-- ============================================================================
--  IMMUNITY CATALOGUE (DEFAULT USED from H5 manual logic)
--  Keys are *spell const strings* (without 'SPELL_' prefix allowed if needed).
--  Each rule contains a set of creature abilities to check and optional
--  predicate flags. If any ability/predicate returns true -> immune.
-- ============================================================================
if not H55_SPELL_IMMUNITIES then H55_SPELL_IMMUNITIES = {} end
-- Helper to add rules by const-string
function _H55__AddImmRule(spellConst, abilitySet, mindLike)
  if not spellConst then return end
  H55_K = spellConst
  if string.sub(H55_K,1,6) ~= 'SPELL_' then H55_K = 'SPELL_'..H55_K end
  H55_SPELL_IMMUNITIES[H55_K] = { abilities = abilitySet, mind = mindLike }
end

-- Generic mind control cluster
H55__MIND_ABS = {
  ABILITY.IMMUNITY_TO_MIND_CONTROL.id, ABILITY.IMMUNITY_TO_HYPNOTIZE.id,
  ABILITY.IMMUNITY_TO_BERSERK.id, ABILITY.IMMUNITY_TO_BLIND.id
}

-- Per-spell rules (extend freely as needed)
_H55__AddImmRule('SPELL_BLIND',           { ABILITY.IMMUNITY_TO_BLIND.id }, true)
_H55__AddImmRule('SPELL_CONFUSION',       H55__MIND_ABS,                    true)
_H55__AddImmRule('SPELL_PUPPET_MASTER',   { ABILITY.IMMUNITY_TO_MIND_CONTROL.id }, true)
_H55__AddImmRule('SPELL_FRENZY',          { ABILITY.IMMUNITY_TO_BERSERK.id }, true)
_H55__AddImmRule('SPELL_SLOW',            { ABILITY.IMMUNITY_TO_SLOW.id },  false)
_H55__AddImmRule('SPELL_WEAKNESS',        { ABILITY.IMMUNITY_TO_CURSE.id }, false)
_H55__AddImmRule('SPELL_DECAY',           { ABILITY.MECHANICAL.id, ABILITY.UNDEAD.id }, false) -- DEFAULT USED
_H55__AddImmRule('SPELL_VULNERABILITY',   { ABILITY.MECHANICAL.id },        false)           -- DEFAULT USED

-- Optional element-oriented blanket immunities (DEFAULT USED)
H55__ELT_FIRE  = { ABILITY.IMMUNITY_TO_FIRE.id }
H55__ELT_WATER = { ABILITY.IMMUNITY_TO_WATER.id }
H55__ELT_EARTH = { ABILITY.IMMUNITY_TO_EARTH.id }
H55__ELT_AIR   = { ABILITY.IMMUNITY_TO_AIR.id }
_H55__AddImmRule('SPELL_FIREBALL',        H55__ELT_FIRE,  false)
_H55__AddImmRule('SPELL_ARMAGEDDON',      H55__ELT_FIRE,  false)
_H55__AddImmRule('SPELL_METEOR_SHOWER',   H55__ELT_EARTH, false)
_H55__AddImmRule('SPELL_LIGHTNING_BOLT',  H55__ELT_AIR,   false)
_H55__AddImmRule('SPELL_CHAIN_LIGHTNING', H55__ELT_AIR,   false)
_H55__AddImmRule('SPELL_ICE_BOLT',        H55__ELT_WATER, false)
_H55__AddImmRule('SPELL_CIRCLE_OF_WINTER',H55__ELT_WATER, false)
_H55__AddImmRule('SPELL_DEEP_FREEZE',     H55__ELT_WATER, false)
_H55__AddImmRule('SPELL_FIREWALL',        H55__ELT_FIRE,  false)

-- ============================================================================
--  PCALL HELPERS (add light variant that returns the raw table from Lua 3.8)
-- ============================================================================
if not _H55_PCallT then
function _H55_PCallT(f, a1,a2,a3,a4,a5,a6,a7,a8,a9)
  if f == nil then return { false, "nil function" } end
  H55_R = pcall(f, a1,a2,a3,a4,a5,a6,a7,a8,a9)
  if type(H55_R) == 'table' then return H55_R end
  return { false, "pcall did not return a table" }
end
end

-- ============================================================================
--  GROWTH: supply *tier defaults* (DEFAULT USED) and override the function
-- ============================================================================
if not H55_TIER_GROWTH_DEFAULT then
  -- DEFAULT USED numbers (vanilla H5 baseline). Replace with MMH55 exact values.
  H55_TIER_GROWTH_DEFAULT = {
    [TIER.ONE]   = 25,  -- T1
    [TIER.TWO]   = 14,  -- T2
    [TIER.THREE] = 9,   -- T3
    [TIER.FOUR]  = 7,   -- T4
...
-- Reverse index from FACTION_UNITS if provided by the map/mod
if not H55_CREATURE_TIER_CACHE then H55_CREATURE_TIER_CACHE = {} end
function _H55__BuildTierCache()
-- Auto-fill per-creature base growth from FACTION_UNITS using tier defaults,
-- but only if an explicit H55_BASE_GROWTH value is not already present.
if not H55_BASE_GROWTH then H55_BASE_GROWTH = {} end
function _H55__AutofillBaseGrowthFromFactionUnits()
  if type(FACTION_UNITS) ~= 'table' then return end
  H55_TOWN = next(FACTION_UNITS, nil)
  while H55_TOWN do
    H55_TIERS = FACTION_UNITS[H55_TOWN]
    H55_T = next(H55_TIERS, nil)
    while H55_T do
      H55_GRADES = H55_TIERS[H55_T]
      H55_G = next(H55_GRADES, nil)
      while H55_G do
        H55_CID = H55_GRADES[H55_G]
        if H55_CID and H55_BASE_GROWTH[H55_CID] == nil then
          H55_BASE_GROWTH[H55_CID] = H55_TIER_GROWTH_DEFAULT[H55_T] or 0
        end
        H55_G = next(H55_GRADES, H55_G)
      end
      H55_T = next(H55_TIERS, H55_T)
    end
    H55_TOWN = next(FACTION_UNITS, H55_TOWN)
  end
end
_H55__AutofillBaseGrowthFromFactionUnits()

  if type(FACTION_UNITS) ~= 'table' then return end
  H55_TOWN = next(FACTION_UNITS, nil)
  while H55_TOWN do
    H55_TIERS = FACTION_UNITS[H55_TOWN]
    if type(H55_TIERS) == 'table' then
      H55_T = next(H55_TIERS, nil)
      while H55_T do
        H55_GRADES = H55_TIERS[H55_T]
        if type(H55_GRADES) == 'table' then
          H55_G = next(H55_GRADES, nil)
          while H55_G do
            H55_CID = H55_GRADES[H55_G]
            if H55_CID then H55_CREATURE_TIER_CACHE[H55_CID] = H55_T end
            H55_G = next(H55_GRADES, H55_G)
          end
        end
        H55_T = next(H55_TIERS, H55_T)
...
_H55__BuildTierCache()

-- Try to detect a creature's tier using engine or the above cache.
if not _H55_GetCreatureTier then
function _H55_GetCreatureTier(creatureToken)
  H55_CID = _H55_Resolve('CREATURE', creatureToken)
  if not H55_CID then return nil end
  -- Try engine function if present
  if type(GetCreatureTier) == 'function' then
    H55_OK, H55_TIER = _H55_PCall(GetCreatureTier, H55_CID)
    if H55_OK and H55_TIER then return H55_TIER end
  end
  -- Try reverse cache from FACTION_UNITS
  H55_T = H55_CREATURE_TIER_CACHE[H55_CID]
  if H55_T then return H55_T end
  return nil
end
end

-- Override growth calculator with tier-default fallback.
function _H55_ComputeGrowth(creatureToken, hasCitadel, hasCastle, hasAsha, multiplier)
  H55_CID = _H55_Resolve('CREATURE', creatureToken)
  H55_BASE = nil

  -- 1) Per-creature override table
  if H55_CID then H55_BASE = H55_BASE_GROWTH[H55_CID] end
  if H55_BASE == nil and type(creatureToken) == 'string' then
    H55_BASE = H55_BASE_GROWTH[_H55_ToUpper(creatureToken)]
  end

  -- 2) Per-tier default
  if H55_BASE == nil then
    H55_T = _H55_GetCreatureTier(creatureToken)
    if H55_T ~= nil and H55_TIER_GROWTH_DEFAULT[H55_T] then
      H55_BASE = H55_TIER_GROWTH_DEFAULT[H55_T]
    end
  end
...
-- Extra: compute directly by tier (no creature required)
function _H55_ComputeGrowthByTier(tierToken, hasCitadel, hasCastle, hasAsha, multiplier)
  H55_T = _H55_ResolveTier(tierToken)
  if H55_T == nil then return 0 end
  H55_BASE = H55_TIER_GROWTH_DEFAULT[H55_T] or 0
...
-- ============================================================================
--  IMMUNITY / STATUS HELPERS
-- ============================================================================
-- Returns true if a unit (battle stack id) is affected by a given spell.
-- Tries several engine names; returns false if unavailable.
if not _H55_IsUnitUnderSpell then
function _H55_IsUnitUnderSpell(unitId, spellToken)
  H55_SID = _H55_Resolve('SPELL', spellToken)
  if not H55_SID then return false end
  -- Try a few engine variants for status queries
  if type(IsCombatUnitAffectedBySpell) == 'function' then
    H55_OK, H55_RES = _H55_PCall(IsCombatUnitAffectedBySpell, unitId, H55_SID)
    if H55_OK then return (H55_RES and H55_RES ~= 0) end
  end
  if type(UnitHasSpell) == 'function' then
    H55_OK, H55_RES = _H55_PCall(UnitHasSpell, unitId, H55_SID)
    if H55_OK then return (H55_RES and H55_RES ~= 0) end
  end
  return false
end
...
-- Returns the base creature id for a combat unit; nil if not available.
if not _H55_GetUnitCreature then
function _H55_GetUnitCreature(unitId)
  if type(GetUnitType) == 'function' then
    H55_OK, H55_RES = _H55_PCall(GetUnitType, unitId)
    if H55_OK then return H55_RES end
  end
  return nil
end
end

-- Creature/Unit ability checks
if not _H55_UnitHasAbility then
function _H55_UnitHasAbility(unitId, abilityToken)
  H55_AID = _H55_Resolve('ABILITY', abilityToken)
  if not H55_AID then return false end
  H55_CID = _H55_GetUnitCreature(unitId)
  if not H55_CID then return false end
  return _H55_CreatureHasAbility(H55_CID, H55_AID)
end
end

-- Mind-immunity predicate used by immunity rules
if not _H55_IsMindImmune then
function _H55_IsMindImmune(unitOrCreature)
  -- treat undead/mechanical as mind-immune (DEFAULT USED), plus explicit abilities
  if _H55_CreatureHasAbility(unitOrCreature, ABILITY.UNDEAD.id) then return true end
  if _H55_CreatureHasAbility(unitOrCreature, ABILITY.MECHANICAL.id) then return true end
  if _H55_CreatureHasAbility(unitOrCreature, ABILITY.IMMUNITY_TO_MIND_CONTROL.id) then return true end
  if _H55_CreatureHasAbility(unitOrCreature, ABILITY.IMMUNITY_TO_HYPNOTIZE.id) then return true end
  return false
end
end

-- Treat a unit as undead while under Vampirism (DEFAULT USED heuristic)
if not _H55_IsEffectivelyUndead then
function _H55_IsEffectivelyUndead(unitIdOrCreature)
  if _H55_CreatureHasAbility(unitIdOrCreature, ABILITY.UNDEAD.id) then return true end
  -- If it's a combat unit, check Vampirism status
  if type(unitIdOrCreature) == 'number' then
    if _H55_IsUnitUnderSpell(unitIdOrCreature, SPELL.DARK.VAMPIRISM) then return true end
  end
  return false
end
end
...
-- ============================================================================
--  CASTING (new canonical entry point) + legacy alias
-- ============================================================================

-- Internal: resolve spell const/name/id to a numeric id
if not _H55__ResolveSpellId then
function _H55__ResolveSpellId(spellIdOrToken)
  -- numeric id directly
  if type(spellIdOrToken) == 'number' then return spellIdOrToken end
  -- Value is already a const-string (e.g., 'SPELL_RESURRECTION') or bare name.
  if type(spellIdOrToken) == 'string' then
    H55_CONST = spellIdOrToken
    if string.sub(H55_CONST,1,6) ~= 'SPELL_' then H55_CONST = 'SPELL_'..H55_CONST end
    H55_ID = _H55_ConstId(H55_CONST)
    if H55_ID then return H55_ID end
  end
  -- Value comes as a table field like SPELL.LIGHT.RESURRECTION == 'SPELL_RESURRECTION'
  if type(spellIdOrToken) == 'table' then
    -- if table is of our enum form { id=?, const=? }, resolve via const if present
    H55_C = spellIdOrToken.const or spellIdOrToken[1]
    if H55_C then
      H55_ID = _H55_ConstId(H55_C)
      if H55_ID then return H55_ID end
    end
  end
  -- Last attempt: generic resolver
  return _H55_Resolve('SPELL', spellIdOrToken)
end
end

-- Returns: ok(bool), err(optional). Prints/flies text when showText is true.
function _H55_CastSpell(casterHero, spellIdOrToken, targetSpec1, targetSpec2, power, showText)
  H55_SID = _H55__ResolveSpellId(spellIdOrToken)
  if not H55_SID then
    print('_H55_CastSpell: unknown spell '..tostring(spellIdOrToken))
    if showText then _H55_Fly(casterHero, 'SPELL FAILED {unknown spell}') end
    return false, 'unknown spell'
  end

  -- If unit target provided, gather creature and immunity data
  H55_TARGET_UNIT = targetSpec1
  H55_TARGET_CID  = nil
  if type(H55_TARGET_UNIT) == 'number' then
    H55_TARGET_CID = _H55_GetUnitCreature(H55_TARGET_UNIT)
  end

  -- Generic magic immunity
  if H55_TARGET_CID and _H55_CreatureHasAbility(H55_TARGET_CID, ABILITY.IMMUNITY_TO_MAGIC.id) then
    if showText then _H55_Fly(casterHero, 'IMMUNE (Magic)') end
    return false, 'immune:magic'
  end

  -- Specific per-spell rules (abilities + mind-like group)
  H55_SCONST = _H55_ConstById('SPELL', H55_SID)
  if H55_SCONST then
    H55_RULE = H55_SPELL_IMMUNITIES[H55_SCONST]
    if not H55_RULE then
      -- allow bare key lookup 'SPELL_XXX' vs 'XXX'
      H55_KEY = string.gsub(H55_SCONST, '^SPELL_', '')
      H55_RULE = H55_SPELL_IMMUNITIES['SPELL_'..H55_KEY]
    end
    if H55_RULE and H55_TARGET_CID then
      -- ability-based checks
      H55_ABS = H55_RULE.abilities
      if type(H55_ABS) == 'table' then
        H55_I = 1
        while H55_ABS[H55_I] do
          if _H55_CreatureHasAbility(H55_TARGET_CID, H55_ABS[H55_I]) then
            if showText then _H55_Fly(casterHero, 'IMMUNE ('..tostring(H55_SCONST)..')') end
            return false, 'immune:ability'
          end
          H55_I = H55_I + 1
        end
      end
      -- mind-like umbrella (undead/mechanical/etc or under Vampirism)
      if H55_RULE.mind then
        if _H55_IsMindImmune(H55_TARGET_CID) or _H55_IsEffectivelyUndead(H55_TARGET_UNIT) then
          if showText then _H55_Fly(casterHero, 'IMMUNE (Mind)') end
          return false, 'immune:mind'
        end
      end
    end
  end

  -- Special-case: raising/resurrection vs undead
  if H55_TARGET_CID then
    if H55_SCONST == 'SPELL_RESURRECTION' or H55_SCONST == 'SPELL_RAISE_DEAD' then
      if _H55_IsEffectivelyUndead(H55_TARGET_UNIT) then
        if showText then _H55_Fly(casterHero, 'IMMUNE (Undead target)') end
        return false, 'immune:undead'
      end
    end
  end

  -- Choose engine entry point, with optional power parameter if available.
  if type(CombatCastSpellWithPower) == 'function' then
    H55_OK, H55_RES = _H55_PCall(CombatCastSpellWithPower, casterHero, H55_SID, power, targetSpec1, targetSpec2)
  elseif type(CombatCastSpellEx) == 'function' then
    -- some builds use an Ex function (spell, power, target1, target2)
    H55_OK, H55_RES = _H55_PCall(CombatCastSpellEx, casterHero, H55_SID, power, targetSpec1, targetSpec2)
  else
    H55_OK, H55_RES = _H55_PCall(CombatCastSpell, casterHero, H55_SID, targetSpec1, targetSpec2)
  end

  if H55_OK then
    if showText then _H55_Fly(casterHero, 'SPELL CAST {'..tostring(H55_SCONST or H55_SID)..'}') end
    return true
  end

  print('_H55_CastSpell: engine rejected: '..tostring(H55_RES))
  if showText then _H55_Fly(casterHero, 'SPELL FAILED {'..tostring(H55_SCONST or H55_SID)..'}') end
  return false, H55_RES
end

-- Legacy alias keeps compatibility and picks new logic.
function _H55_SafeCastSpell(casterHero, spellIdOrToken, targetSpec1, targetSpec2, power, showText)
  return _H55_CastSpell(casterHero, spellIdOrToken, targetSpec1, targetSpec2, power, showText)
end

-- Explicit helper: check if a given unit/creature is immune to a spell
-- Returns true/false and a reason string.
function _H55_IsImmuneToSpell(unitOrCreature, spellIdOrToken)
  H55_SID = _H55__ResolveSpellId(spellIdOrToken)
  if not H55_SID then return false, 'unknown spell' end
  H55_SCONST = _H55_ConstById('SPELL', H55_SID) or 'SPELL_UNKNOWN'
  H55_RULE = H55_SPELL_IMMUNITIES[H55_SCONST]
  if not H55_RULE then return false, 'no rule' end

  -- normalize the target as creature id
  H55_CID = unitOrCreature
  if type(unitOrCreature) == 'number' and unitOrCreature > 0 and unitOrCreature < 10000 then
    -- treat as unit id first
    H55_X = _H55_GetUnitCreature(unitOrCreature)
    if H55_X then H55_CID = H55_X end
  end

  if type(H55_CID) ~= 'number' then return false, 'not a creature' end

  -- magic blanket
  if _H55_CreatureHasAbility(H55_CID, ABILITY.IMMUNITY_TO_MAGIC.id) then
    return true, 'immune:magic'
  end
  ...
  if type(H55_ABS) == 'table' then
    H55_I = 1
    while H55_ABS[H55_I] do
      if _H55_CreatureHasAbility(H55_CID, H55_ABS[H55_I]) then return true, 'immune:ability' end
      H55_I = H55_I + 1
    end
  end
  if H55_RULE.mind then
    if _H55_IsMindImmune(H55_CID) then return true, 'immune:mind' end
  end
  return false, 'ok'
end


-- Cache: remember 'const name' for resolved spell ids (to enable _H55_ConstById on 'SPELL')
if not H55_SPELL_CONST_CACHE then H55_SPELL_CONST_CACHE = {} end

-- Extend _H55__ResolveSpellId to backfill the cache for pretty messages.
H55__OLD__ResolveSpellId = _H55__ResolveSpellId
function _H55__ResolveSpellId(spellIdOrToken)
  -- Call the actual resolver
  H55_ID = H55__OLD__ResolveSpellId(spellIdOrToken)
    -- Try to infer const name and remember it
    if H55_ID then
      -- direct const string passed
      if type(spellIdOrToken) == 'string' then
        H55_C = spellIdOrToken
        if string.sub(H55_C,1,6) ~= 'SPELL_' then H55_C = 'SPELL_'..H55_C end
        H55_SPELL_CONST_CACHE[H55_ID] = H55_C
      end
      -- table value that contains a const field
      if type(spellIdOrToken) == 'table' then
        H55_C2 = spellIdOrToken.const or spellIdOrToken[1]
        if type(H55_C2) == 'string' then H55_SPELL_CONST_CACHE[H55_ID] = H55_C2 end
      end
    end
    return H55_ID
  end
...
-- Add 'SPELL' support to _H55_ConstById via cache (non-invasive).
H55__OLD__ConstById = _H55_ConstById
function _H55_ConstById(domain, id)
  if domain == 'SPELL' then
      return H55_SPELL_CONST_CACHE[id]
    end
    return H55__OLD__ConstById(domain, id)
end

-- ============================================================================
--  SKILL (alias) — dot-notation for base hero skills (DEFAULT USED)
--  Values are const strings ('HERO_SKILL_*') resolved lazily at runtime.
-- ============================================================================
if not SKILL then SKILL = {} end
SKILL.ATTACK                = 'HERO_SKILL_ATTACK'
SKILL.DEFENSE               = 'HERO_SKILL_DEFENCE'   -- some builds use DEFENCE spelling
SKILL.DEFENCE               = 'HERO_SKILL_DEFENCE'   -- synonym
SKILL.LOGISTICS             = 'HERO_SKILL_LOGISTICS'
SKILL.LUCK                  = 'HERO_SKILL_LUCK'
SKILL.LEADERSHIP            = 'HERO_SKILL_LEADERSHIP'
SKILL.ENLIGHTENMENT         = 'HERO_SKILL_ENLIGHTENMENT'
SKILL.SORCERY               = 'HERO_SKILL_SORCERY'
SKILL.WAR_MACHINES          = 'HERO_SKILL_WAR_MACHINES'
SKILL.SUMMONING_MAGIC       = 'HERO_SKILL_SUMMONING_MAGIC'
SKILL.DESTRUCTIVE_MAGIC     = 'HERO_SKILL_DESTRUCTIVE_MAGIC'
SKILL.LIGHT_MAGIC           = 'HERO_SKILL_LIGHT_MAGIC'
SKILL.DARK_MAGIC            = 'HERO_SKILL_DARK_MAGIC'
SKILL.SHATTER_DARK          = 'HERO_SKILL_SHATTER_DARK_MAGIC'
SKILL.SHATTER_DESTRUCTIVE   = 'HERO_SKILL_SHATTER_DESTRUCTIVE_MAGIC'
SKILL.SHATTER_LIGHT         = 'HERO_SKILL_SHATTER_LIGHT_MAGIC'
SKILL.SHATTER_SUMMONING     = 'HERO_SKILL_SHATTER_SUMMONING_MAGIC'
